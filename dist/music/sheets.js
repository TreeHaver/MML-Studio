...
export function createSheetPlanner(project, index, limit, extremeCompression = false, minimumEnd = 0) {
    ...
    const next = (start) => {
        if (start < 0 || start >= end || !Number.isSafeInteger(start))
            throw Error('Invalid part start.');
        const cache = new Map();
        const at = (stop) => { let r = cache.get(stop); if (!r) {
            r = render(start, stop);
            cache.set(stop, r);
        } return r; };
        if (at(end).bytes <= limit)
            return at(end);
        ...
        for (let tick = Math.min(end - 1, Math.max(best + 32, start + 128)); tick > Math.max(start, best - 32); tick--) {
            if (at(tick).bytes <= limit) {
                best = Math.max(best, tick);
                break;
            }
        }
        if (best === start)
            throw Error(`Character limit ${limit} is too small to fit the next synchronized notes and their tempo/volume commands. Increase the limit or export in a single file.`);
        ...
        for (const n of source) {
            for (const t of [n.start, n.start + n.length])
                if (t > start && t <= best && t >= best - 32)
                    candidates.add(t);
        }
        for (const tick of [...candidates].sort((a, b) => b - a)) {
            if (!source.some(n => n.start < tick && n.start + n.length > tick) && at(tick).bytes <= limit)
                return at(tick);
        }
        return at(best);
    };
    return { whole, end, render, next, sourceTick: expanded.sourceTick, split: () => {
            ...
            if (whole.bytes <= limit)
                return [{ ...whole, start: 0, end }];
            const parts = [];
            let start = 0;
            while (start < end) {
                const part = next(start);
                parts.push(part);
                start = part.end;
            }
            return parts;
...
export function ms2Xml(channels) {
