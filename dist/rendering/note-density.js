...
export function drawOverlapMarkers() {
    if (overlapNotes !== state.project.notes || overlapCount !== state.project.notes.length || overlapInstruments !== state.project.instruments || overlapActive !== state.active) {
        ...
        overlapInstruments = state.project.instruments;
...
const span = (region) => ({ left: KEY + region.start * state.zoom - view.scrollLeft, right: KEY + region.end * state.zoom - view.scrollLeft });
...
export function drawCrowdedRegions() {
    ...
    for (const region of current()) {
        ...
        if (right <= KEY || left >= state.width)
            continue;
        ctx.fillStyle = '#f0b90016';
        ctx.fillRect(left, HEAD, right - left, state.height - HEAD);
        ctx.fillStyle = '#d9a40040';
        ctx.fillRect(left, HEAD, 1, state.height - HEAD);
        ctx.fillRect(right - 1, HEAD, 1, state.height - HEAD);
    }
    ctx.restore();
}
