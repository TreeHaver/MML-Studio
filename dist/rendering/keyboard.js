import { palette, keyboardView } from '../appearance.js';
...
export function drawKeyboard() {
    ...
    if (keyboardView.piano)
        piano(firstPitch, lastPitch);
    else
        names(firstPitch, lastPitch);
    ctx.restore();
}
const top = (pitch) => HEAD + pitchTop(state.topPitch, pitch) - view.scrollTop;
function names(firstPitch, lastPitch) {
 ...
 */
function piano(firstPitch, lastPitch) {
    ctx.fillStyle = palette.pianoWhite;
    ctx.fillRect(0, 0, KEY, state.height);
    ctx.strokeStyle = palette.pianoLine;
    ctx.lineWidth = 1;
    ctx.beginPath();
    for (let p = firstPitch; p >= lastPitch; p--) {
        if (sharp(p))
    ...
    for (let p = firstPitch; p >= lastPitch; p--) {
        ...
        if (sharp(p)) {
            ctx.fillStyle = active ? palette.playhead : palette.pianoBlack;
            ctx.fillRect(0, y, BLACK, height);
        }
        else if (active) {
            ctx.fillStyle = palette.playhead;
            ctx.fillRect(0, y, KEY, height);
        }
    }
    // Only the Cs are named. Labelling every key would turn the keyboard back into a list.
    ctx.fillStyle = palette.pianoLabel;
    ctx.font = '10px Segoe UI';
    ctx.textAlign = 'right';
    ctx.textBaseline = 'middle';
    for (let p = firstPitch; p >= lastPitch; p--)
        if (p % 12 === 0 && p !== state.previewPitch)
            ctx.fillText(name(p), KEY - 6, top(p) + pitchHeight(p) / 2);
    ctx.textAlign = 'left';
}
