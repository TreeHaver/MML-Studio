...
export function drawTempoMarkers() {
    ...
    const label = (tick, text) => { const parts = labels.get(tick) ?? new Set(); parts.add(text); labels.set(tick, parts); };
    for (const n of state.project.notes)
        if (state.project.instruments[n.instrument].isInstructions && !isMuted(n.instrument)) {
            if (n.section?.trim())
                label(n.start, n.section.trim());
            if (n.timeSignature)
                label(n.start, n.timeSignature);
            if (n.section?.trim() && n.resetMeasures)
                label(n.start, 'Measure 1');
        }
    ...
    for (const event of tempoChanges(state.project.notes)) {
        if (!visible.has(`${event.tick}:${event.bpm}`))
            continue;
    ...
    for (const [tick, parts] of [...labels].sort((a, b) => a[0] - b[0])) {
        const x = KEY + tick * state.zoom - view.scrollLeft;
        ...
        if (row < 0)
            row = rowEnds.length;
        rowEnds[row] = x + width + 8;
        const y = HEAD + 4 + row * 24;
        ctx.fillStyle = palette.ruler;
        ctx.fillRect(x + 3, y, width, 20);
        ctx.strokeStyle = INSTRUCTIONS_COLOR;
        ctx.lineWidth = 1;
        ctx.strokeRect(x + 3.5, y + .5, width - 1, 19);
