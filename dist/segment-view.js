...
import { $, view, ctx, status } from './dom.js';
import { playback, stopPlayback, seekToTick } from './playback/transport.js';
import { rangeAt, sectionMarkers } from './model/segment-view.js';
import { enterSegment, fullProject, resetSegment } from './segment-session.js';
...
function clearSelection() { state.selection.clear(); state.gesture = null; $('section-nav').value = ''; instrumentView.mmlEpoch++; setPastePosition(0); }
export function openSegmentView(kind) {
    if (state.gesture)
        return;
    try {
        const root = fullProject(), tick = (state.segment?.projection.range.start ?? 0) + (playback.tick ?? 0), range = rangeAt(root, tick, kind);
        if (!range) {
            status(`Place the playhead inside a ${kind} first.`);
            return;
        }
        stopPlayback(false);
        enterSegment(root, range);
        clearSelection();
        view.scrollLeft = 0;
        refresh();
        seekToTick(0);
        ...
        status('Could not open view: ' + error);
    }
}
export function returnToProject() {
    if (!state.segment || state.gesture)
        return;
    try {
        const tick = state.segment.projection.range.start + (playback.tick ?? 0), root = fullProject();
        stopPlayback(false);
        resetSegment();
        state.project = root;
        clearSelection();
        refresh();
        seekToTick(tick);
        view.scrollLeft = tick * state.zoom;
        status('Returned to the full project.');
    }
    catch (error) {
        status('Could not return to project: ' + error);
    }
}
export function drawSegmentBoundary() {
    ...
    ctx.clip();
    ctx.fillStyle = '#66778835';
    ctx.fillRect(Math.max(KEY, x), HEAD, state.width - Math.max(KEY, x), state.height - HEAD);
    ctx.fillStyle = '#718096';
    ctx.fillRect(x, HEAD, 2, state.height - HEAD);
    ctx.font = '11px Segoe UI';
    ctx.textBaseline = 'top';
    ctx.fillText('End of ' + state.segment.projection.range.kind, x + 5, HEAD + 5);
    ctx.restore();
}
