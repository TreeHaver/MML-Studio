import { secondsAtTick } from '../music/tempo.js';
 ...
 */
export function metronomeBeats(project, map, end) {
    if (!(end > 0))
        return [];
...
let beats = [], cursor = 0, lastHeard = -1;
const pending = new Set();
/** Half a second of clicks is prepared in advance, which is what keeps them steady. */
const LOOKAHEAD = .25;
export function setMetronomeBeats(next) { beats = next; cursor = 0; lastHeard = -1; }
...
function click(context, when, major) {
    const level = metronome.volume * (major ? .5 : .32);
    if (level <= 0)
        return;
    const osc = context.createOscillator(), gain = context.createGain();
    osc.type = 'sine';
    osc.frequency.value = major ? 1600 : 1050;
    ...
    osc.stop(when + .09);
    pending.add(osc);
    osc.onended = () => { pending.delete(osc); gain.disconnect(); };
    metronome.scheduled++;
}
/**
 ...
 */
export function scheduleMetronome(context, heard, speed) {
    if (!metronome.on || !beats.length)
        return;
    if (lastHeard < 0 || heard < lastHeard - .05 || heard > lastHeard + 1) {
        clearMetronome();
        cursor = beatIndexAt(beats, heard);
    }
    lastHeard = heard;
