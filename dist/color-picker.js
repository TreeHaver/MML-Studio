...
function open(swatch, pick) {
    ...
    panel.className = 'menu-panel color-panel';
    const area = document.createElement('div');
    area.className = 'color-area';
    const areaDot = document.createElement('div');
    areaDot.className = 'color-dot';
    area.append(areaDot);
    const hue = document.createElement('div');
    hue.className = 'color-hue';
    const hueDot = document.createElement('div');
    hueDot.className = 'color-dot';
    hue.append(hueDot);
    ...
    const show = (editing = false) => {
        ...
        areaDot.style.left = s * 100 + '%';
        areaDot.style.top = (1 - v) * 100 + '%';
        ...
        for (const cell of cells)
            cell.classList.toggle('current', same(cell.title, color));
    };
    const commit = (color) => {
    ...
    const track = (element, to) => {
        ...
        element.onpointermove = event => { if (element.hasPointerCapture(event.pointerId))
            apply(event); };
        element.onpointerup = event => { if (!element.hasPointerCapture(event.pointerId))
            return; element.releasePointerCapture(event.pointerId); apply(event); commit(current()); };
    };
    track(area, (x, y) => { s = x; v = 1 - y; });
    track(hue, x => { h = x * 360; });
    hex.oninput = () => { if (valid(hex.value)) {
        const hsv = hexToHsv(hex.value.trim());
        h = hsv.h;
        s = hsv.s;
        v = hsv.v;
        show(true);
    ...
    const outside = (event) => { const target = event.target; if (target !== swatch && !target?.closest?.('.color-panel'))
        shut(); };
    const dismiss = (event) => { if (event.key === 'Escape')
        shut(); };
    function shut() {
        panel.remove();
        ...
        document.removeEventListener('keyup', dismiss, true);
    }
    document.addEventListener('pointerdown', outside, true);
    document.addEventListener('keyup', dismiss, true);
    close = shut;
}
