...
import { colorSwatch, closeColorPanel } from './color-picker.js';
import { instrumentActions, removeInstrument } from './instrument-actions.js';
import { GM_PROGRAMS } from './playback/gm-programs.js';
import { VANILLA_PROGRAMS } from './playback/vanilla-instruments.js';
import { DRUM_KIT_NAME, DRUM_MS2_WARNING, MS2_DRUMS } from './playback/drums.js';
import { syncAdvancedInstructions, instructionCard } from './advanced-instructions.js';
...
export function instruments() {
    ...
    const shown = (name) => !query || name.toLowerCase().includes(query);
    $('instruments').replaceChildren();
    state.project.instruments.forEach((i, index) => {
        if (i.isInstructions) {
            if (shown(i.name))
                $('instruments').append(instructionCard(index));
            return;
        }
        if (!shown(i.name))
            return;
        ...
        preset.title = 'Playback instrument';
        ...
        const choices = [];
        const buildOption = (choice) => {
            const option = document.createElement('option');
            option.value = choice.value;
            option.textContent = choice.label;
            // Keep an excluded current value in the closed field, but not among selectable choices.
            option.hidden = choice.excluded;
            option.disabled = choice.excluded;
            if (choice.note)
                option.title = choice.note;
            option.classList.toggle('preset-warning', choice.value === 'drums' || choice.excluded);
            return option;
        };
        const addPreset = (value, label, vanilla, note = '') => {
            const excluded = instrumentView.vanillaOnly && !vanilla;
            if (excluded && value !== selected)
                return;
            const choice = { value, label, excluded, note };
            choices.push(choice);
            if (value === selected) {
                preset.append(buildOption(choice));
                preset.classList.toggle('preset-warning', value === 'drums' || excluded);
            }
        };
        ...
        for (const [key, drum] of Object.entries(MS2_DRUMS)) {
            const choice = { value: key, label: drum.name, excluded: false, note: '' };
            choices.push(choice);
            if (key === selected)
                preset.append(buildOption(choice));
        }
        ...
        preset.fillOptions = () => {
            if (preset.children.length >= choices.length)
                return;
            preset.replaceChildren(...choices.map(buildOption));
            preset.value = selected;
        };
        preset.value = selected;
        preset.onchange = () => { if (preset.value !== 'drums' && !(preset.value in MS2_DRUMS) && !/^\d+$/.test(preset.value))
            return; checkpoint(); delete i.ms2Drum; if (preset.value in MS2_DRUMS)
            i.ms2Drum = preset.value; i.isDrum = preset.value === 'drums'; i.midiProgram = i.isDrum || i.ms2Drum ? 0 : Number(preset.value); if (i.ms2Drum)
        ...
        mute.title = (muted ? 'Unmute ' : 'Mute ') + i.name;
        mute.setAttribute('aria-label', mute.title);
        mute.setAttribute('aria-pressed', String(muted));
        mute.classList.toggle('active', muted);
        mute.onclick = () => { if (muted)
            instrumentView.muted.delete(index);
        else {
            instrumentView.muted.add(index);
            if (soloed)
        ...
        remove.title = 'Delete ' + i.name;
        remove.setAttribute('aria-label', 'Delete ' + i.name);
        ...
        rename.className = 'instrument-row-rename';
        rename.title = 'Rename ' + i.name;
        rename.setAttribute('aria-label', 'Rename ' + i.name);
    ...
    if (!state.project.instruments.some(i => i.isInstructions) && shown('Instructions'))
        $('instruments').append(instructionCard());
    $('instrument-empty').hidden = !!$('instruments').children.length || !query;
...
export function installInstruments() {
    ...
    $('collapse-all').onclick = () => {
        ...
        if (anyOpen)
            for (const index of musical)
                instrumentView.collapsed.add(index);
        else
            instrumentView.collapsed.clear();
