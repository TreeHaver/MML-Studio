...
function validateVoiceCount(voices) { if (!Number.isSafeInteger(voices) || voices < 1)
    throw Error('Enter a positive whole number of voices.'); }
...
export function audioSampling(intervalMs = 30) {
    if (!Number.isFinite(intervalMs) || intervalMs < 1.875)
        throw Error('Enter a sample interval of at least 1.875 ms.');
    const ticks = Math.round(intervalMs / 1.875);
    if (!Number.isSafeInteger(ticks) || !Number.isSafeInteger(ticks * 30))
        throw Error('The sample interval is too large.');
    return { ticks, hop: ticks * 30, intervalMs: ticks * 1.875 };
}
export function estimateAudioCharacters(seconds, intervalMs = 30, voices = 5) {
    validateVoiceCount(voices);
    if (!Number.isFinite(seconds) || seconds < 0)
        throw Error('Invalid clip duration.');
...
function spectrum(samples, center) {
    const real = new Float64Array(SIZE), imag = new Float64Array(SIZE);
    for (let i = 0; i < SIZE; i++)
        real[i] = (samples[center + i - SIZE / 2] ?? 0) * (0.5 - 0.5 * Math.cos(2 * Math.PI * i / (SIZE - 1)));
...
function peaks(samples, center, voices) {
    const bins = spectrum(samples, center), byPitch = new Map();
    for (let i = 6; i < Math.min(bins.length - 1, Math.floor(5000 * SIZE / RATE)); i++) {
        if (bins[i] <= bins[i - 1] || bins[i] < bins[i + 1] || bins[i] < 1e-10)
            continue;
        ...
        if (!byPitch.has(pitch) || byPitch.get(pitch).score < score)
            byPitch.set(pitch, { pitch, amplitude, score });
    }
...
export function importAudio(samples, sampleRate, name, intervalMs = 30, budget = 10000, voices = 5) {
    validateVoiceCount(voices);
    if (sampleRate !== RATE || !samples.length)
        throw Error('Audio analysis requires nonempty 16 kHz mono PCM.');
    const sampling = audioSampling(intervalMs), HOP = sampling.hop, TICKS = sampling.ticks;
    for (const sample of samples)
        if (!Number.isFinite(sample))
            throw Error('The decoded audio contains invalid samples.');
    const frames = [];
    let reference = 0, maxRms = 0;
    for (let start = 0; start < samples.length; start += HOP) {
        ...
        for (const peak of found)
            reference = Math.max(reference, peak.amplitude);
    ...
    const previous = Array(VOICES).fill(undefined);
    for (let frame = 0; frame < frames.length; frame++) {
        const start = LEAD_IN + frame * TICKS, length = Math.min(TICKS, endTick - start);
        if (length <= 0)
            break;
        ...
        for (const pair of pairs)
            if (pair.distance <= 12 && !assigned.has(pair.voice) && !used.has(pair.index)) {
                assigned.set(pair.voice, candidates[pair.index]);
                used.add(pair.index);
            }
        for (let index = 0; index < candidates.length; index++)
            if (!used.has(index)) {
                const voice = Array.from({ length: VOICES }, (_, i) => i).find(i => !assigned.has(i));
                assigned.set(voice, candidates[index]);
            }
        for (let voice = 0; voice < VOICES; voice++) {
            ...
            if (last && last.start + last.length === start && last.pitch === candidate.pitch && last.volume === candidate.volume)
                last.length += length;
            else {
                const note = { id: id++, instrument: 0, start, length, pitch: candidate.pitch, volume: candidate.volume };
    ...
    if (project.notes.length === 2)
        throw Error('No usable voice frequencies were found in this audio.');
    ...
    if (musicalEnd < performanceEnd)
        project.notes.push({ id: id++, instrument: 0, start: musicalEnd, length: performanceEnd - musicalEnd, pitch: 60, volume: 0 });
