...
export function importMidi(bytes) {
    ...
    function close(n, tick) {
        ...
        if (tick <= n.tick)
            warnings.add('Zero-duration notes were expanded to one timing unit.');
    ...
    const completed = [];
    const finish = (note, end) => { completed.push({ note, end }); };
    for (const e of midi.events) {
        if (e.status === 255) {
            if (e.meta === 81) {
                ...
                if (Math.abs(raw - bpm) > 1e-8)
                    warnings.add('MIDI tempo was rounded to the nearest whole BPM for integer tempo instructions.');
                if (tempos.has(tick) && tempos.get(tick) !== bpm)
        ...
        if (e.status >= 240) {
            warnings.add('System-exclusive messages are not imported.');
            continue;
        }
        const channel = e.status & 15, key = `${e.port}:${channel}`, kind = e.status >> 4;
        let c = channels.get(key);
        if (!c) {
            c = { program: 0, sustain: false, down: new Map(), held: [] };
            channels.set(key, c);
        }
        const flushHeld = () => { for (const n of c.held)
            finish(n, e.tick); c.held = []; };
        if (kind === 12) {
            c.program = e.data[0];
            continue;
        }
        if (kind === 9 && e.data[1] > 0) {
            ...
            const queue = c.down.get(n.pitch) ?? [];
            queue.push(n);
            c.down.set(n.pitch, queue);
            if (channel === 9) {
                warnings.add(`${DRUM_KIT_NAME}: ${DRUM_MS2_WARNING}`);
                if (c.program !== 0)
                    warnings.add('Alternate MIDI drum kits use the Standard Drum Kit for preview.');
            }
        }
        else if (kind === 8 || kind === 9) {
            const queue = c.down.get(e.data[0]), n = queue?.shift();
            if (!queue?.length)
                c.down.delete(e.data[0]);
            if (n) {
                if (c.sustain)
                    c.held.push(n);
                else
                ...
                warnings.add('Unmatched note-off events were ignored.');
        }
        else if (kind === 11) {
            ...
            if (cc === 64) {
                ...
                for (const queue of c.down.values())
                ...
                warnings.add('Controllers other than sustain and note/reset controls are not imported (including bank, volume, pan and expression).');
        }
        else
            warnings.add('Pitch bend and aftertouch are not imported.');
    }
    for (const c of channels.values()) {
        const remaining = [...c.down.values()].flat().concat(c.held);
        if (remaining.length)
            warnings.add('Unreleased notes were ended at the end of the MIDI file.');
    ...
    for (const n of project.notes)
        if (!firstAt.has(n.start))
            firstAt.set(n.start, n);
    for (const [tick, bpm] of [...tempos].sort((a, b) => a[0] - b[0])) {
        if (bpm === previous && (completed.length > 0 || project.notes.length > 0))
            continue;
        previous = bpm;
