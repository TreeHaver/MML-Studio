import { fresh, colors } from '../model/project.js';
...
export function importMml(text, name = 'MML') {
    ...
    if (/^\s*(?:<\?xml|<ms2\b)/i.test(text)) {
        ...
        if (body.trim() || !channels.length)
            throw Error('Invalid or unsupported MS2MML XML elements.');
        ...
        for (const block of blocks) {
            ...
            if (program > 127)
                throw Error('Unsupported MNE instrument ' + program);
        ...
        for (const section of sections) {
            const number = section[1].match(/^Channel(\d+)$/i)?.[1];
            if (!number)
                continue;
        ...
        parts.push({ name, channels: wrapped(strip(text)) });
    if (!parts.length)
        throw Error('No MML parts found.');
    for (const part of parts) {
        ...
        for (const raw of part.channels) {
            const s = strip(raw).replace(/\s/g, '').toLowerCase();
            let p = 0, tick = 0, octave = 4, volume = 8, defaultLength = 32, tie = false, last;
            const number = (required = false) => { const m = s.slice(p).match(/^\d+/); if (!m) {
                if (required)
                    throw Error('Expected number at ' + p);
                return undefined;
            } p += m[0].length; const n = Number(m[0]); if (!Number.isSafeInteger(n))
                throw Error('Number exceeds timing precision.'); return n; };
            const length = () => { const d = number(); if (d === 0)
                throw Error('Length denominator must be positive.'); let n = d === undefined ? defaultLength : 128 / d, extra = n / 2; while (s[p] === '.') {
                n += extra;
                extra /= 2;
                p++;
            } return n; };
            while (p < s.length) {
                const c = s[p++];
                if (c === 'o') {
                    const sign = s[p] === '-' ? (p++, -1) : 1;
                    octave = sign * number(true);
                    continue;
                }
                if (c === '<' || c === '>') {
                    octave += c === '>' ? 1 : -1;
                ...
                if (c === 'v') {
                    volume = number(true);
                    if (volume > 15)
                        throw Error('Volume outside model range 0–15.');
                    continue;
                }
                if (c === 't') {
                    const bpm = number(true);
                    if (bpm < 1)
                ...
                if (!'cdefgabnr'.includes(c))
                    throw Error('Unsupported MML token ' + JSON.stringify(c) + ' at ' + (p - 1));
                let pitch = c === 'n' ? number(true) : (octave + 1) * 12 + ({ c: 0, d: 2, e: 4, f: 5, g: 7, a: 9, b: 11 }[c] ?? 0);
                if (c !== 'n' && c !== 'r' && ['+', '#', '-'].includes(s[p]))
                    pitch += s[p++] === '-' ? -1 : 1;
                const duration = c === 'n' ? defaultLength : length(), start = Math.round(tick), end = Math.round(tick + duration);
                if (!Number.isSafeInteger(end) || end <= start)
                    throw Error('Duration is finer than the version-2 timing model can represent.');
                if (Math.abs(tick - start) > 1e-7 || Math.abs(tick + duration - end) > 1e-7)
                    warnings.add('Fractional timing rounded to the nearest 1/128 whole-note unit; no grid snapping applied.');
                if (c === 'r') {
                    if (tie)
                        throw Error('Cannot tie a rest.');
                    last = undefined;
                }
                else if (tie) {
                    if (!last || last.pitch !== pitch || last.start + last.length !== start)
                        throw Error('Tie must continue the same adjacent pitch.');
                    if (last.volume !== volume)
                        warnings.add('Volume changes inside tied notes cannot be represented and retain the initial note volume.');
                    last.length = end - last.start;
                    tie = false;
                }
                else {
                    last = { id: ++id, instrument, start, length: end - start, pitch, volume };
                    project.notes.push(last);
                }
                tick += duration;
            }
            if (tie)
                throw Error('Unfinished tie.');
            span = Math.max(span, Math.round(tick));
        }
    }
    if (!project.notes.length && !tempos.size)
        throw Error('No notes or tempo instructions found.');
    for (const [start, tempo] of tempos) {
        const note = project.notes.find(n => n.start === start);
        if (note)
            note.tempo = tempo;
        else
    ...
    for (const n of project.notes) {
        if (project.instruments[n.instrument].isInstructions)
            continue;
        const key = n.instrument + ':' + n.start;
        const prior = volumes.get(key);
        if (prior !== undefined && prior !== n.volume)
            warnings.add('Simultaneous channels have different volumes. Notes retain explicit volumes, but this model resolves one volume per instrument/position for playback and export.');
        volumes.set(key, n.volume);
    }
    if (!valid(project.notes))
        throw Error('Invalid imported timing or tempo.');
...
function wrapped(text) {
    ...
    if (/^mml@/i.test(s)) {
        if (!s.endsWith(';'))
            throw Error('MML@ must end with a semicolon.');
        return s.slice(4, -1).split(',');
    }
