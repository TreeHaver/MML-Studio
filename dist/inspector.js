...
export function info() {
    ...
    for (const key of ['pitch', 'length', 'volume'])
        $(key + '-field').hidden = !!instructions;
    $('visual-instructions').hidden = !instructions;
    for (const [id, key] of [['loop-entry', 'loopEntry'], ['loop-exit', 'loopExit'], ['loop-tie', 'loopTie']]) {
        input(id).checked = !!n?.[key];
        input(id).disabled = !instructions;
    }
    input('loop-count').value = String(n?.loopCount ?? 1);
    ...
    input('time-signature').value = n?.timeSignature ?? '';
    input('section-name').value = n?.section ?? '';
    input('section-reset').checked = !!n?.resetMeasures;
    input('section-reset').disabled = !instructions;
    if (n) {
        input('pitch').value = String(n.pitch);
        input('length').value = String(n.length);
        input('volume').value = n.volume === null ? '' : String(n.volume);
        input('tempo').value = n.tempo == null ? '' : String(n.tempo);
    }
}
export function installInspector() {
    for (const [id, key] of [['loop-entry', 'loopEntry'], ['loop-exit', 'loopExit'], ['loop-tie', 'loopTie'], ['loop-count', 'loopCount']])
        $(id).onchange = () => {
            ...
            if (!n || !state.project.instruments[n.instrument].isInstructions)
                return;
            if (key === 'loopCount' && !n.loopEntry || key === 'loopTie' && !n.loopExit)
                return;
            const value = key === 'loopCount' ? Number(input(id).value) : input(id).checked;
            if (key === 'loopCount' && (!Number.isSafeInteger(value) || Number(value) < 1)) {
                status('Loop Count must be a positive whole number.');
                info();
                return;
            }
    ...
    $('section-reset').onchange = () => { const n = anchor(); if (!n || !state.project.instruments[n.instrument].isInstructions)
        return; const resetMeasures = input('section-reset').checked; commitNotes(state.project.notes.map(o => state.selection.has(o.id) && state.project.instruments[o.instrument].isInstructions ? { ...o, resetMeasures } : o)); };
    for (const [id, key] of [['time-signature', 'timeSignature'], ['section-name', 'section']])
        $(id).onchange = () => {
            const n = anchor();
            if (!n || !state.project.instruments[n.instrument].isInstructions)
                return;
    ...
    for (const key of ['pitch', 'length', 'volume', 'tempo'])
        $(key).onchange = () => {
            const n = anchor();
            if (!n || state.project.instruments[n.instrument].isInstructions && key !== 'tempo')
                return;
            const value = input(key).value;
            const num = Number(value);
            if (key === 'tempo' && !validTempo(value === '' ? null : num)) {
                status('Tempo must be a positive whole number of BPM.');
                info();
                return;
            }
            if (key !== 'tempo' && key !== 'volume' && !Number.isInteger(num)) {
                info();
                return;
            }
            const next = state.project.notes.map(o => state.selection.has(o.id) ? { ...o, [key]: (key === 'volume' || key === 'tempo') ? (value === '' ? null : num) : key === 'pitch' ? o.pitch + num - n.pitch : num } : o);
            commitNotes(next);
        };
}
