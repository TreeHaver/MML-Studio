...
The app payload includes compiled modules, HTML/CSS, images, the SoundFont, bundled audio code and license notices. Development documentation, tests, source TypeScript, npm dependencies, build scripts and previous archives are excluded. Electron's own runtime DLLs, locales and notices are retained. The executable is a renamed copy of the installed Electron runtime with the blue note-M icon from `assets/logo.ico` embedded by the build-only `build-icon.cs` helper. The helper also sets the executable description/product name to MML Music Studio and its version to the project version. This creates a portable folder, not an installer; release signing is not configured.
