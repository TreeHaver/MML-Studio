...
Sound-file conversion is not supported. Dropped MP3/WAV and other audio files are rejected without changing the project. MIDI/MML imports retain integer 1/128-whole-note model resolution: supported fractional source timing rounds with a notice, and zero-length results fail. The user's in-game observation that Speed Multiplier output can use much finer timing is documented in MUSIC_MODEL; it changes neither import resolution nor the editing grid.
