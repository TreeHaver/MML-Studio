...
## Edge scrolling while moving notes and selection-based paste — 2026-09-08

Dragging selected notes now scrolls horizontally near either visible roll edge, using the same edge zone and speed as box selection. Each animation frame recomputes group movement from the original notes and viewport offset, so holding the pointer still continues moving the group with the screen. The scroll extent updates as notes move. Relative timing, duration, pitches, snapping and Instructions behavior remain intact. Release, cancellation, lost pointer capture and Escape stop the animation; a completed move is one undoable edit. Box selection retains both-axis scrolling.

Internal note paste now starts at the greatest end timestamp among selected notes in the active Instrument. Each paste becomes the selection, so subsequent pastes append after it. With no selected notes in that Instrument, paste uses the copied group's original earliest timestamp: copy/delete/paste restores its placement, and changing Instrument instances layers the copy at the original timestamps. Pitch and selection order do not determine placement. Existing inherited-volume preservation, tempo conflict checks and scoped-view validation remain. Empty-space insertion positions still apply to external MML text, not internal note copies.

Changed: src/pointer.ts, src/note-clipboard.ts, dist/pointer.js, dist/note-clipboard.js, tests/renderer.test.cjs, tests/electron-pitch-layout.cjs, PROGRESS.md.

Actual validation: incremental node build.cjs passed; full node tests/run.cjs passed all 117 tests, including renderer integration. Expanded simulated renderer coverage checks stationary-pointer scrolling in both directions, group offsets/durations/pitch, threshold, cancellation, one-step Undo, box scrolling, repeated paste, original nonzero copy position after deletion, latest end independent of pitch/onset/order, switching Instrument instances, V0 and tempo conflict rejection. Native electron-pitch-layout passed its existing drawing, selection, movement, resizing and preview checks plus stationary edge dragging left/right at horizontal zoom 3 and 6, preservation of group offsets and note properties, and stopping on release. The sandboxed build/test runner hit the known esbuild directory access failure; sandboxed Electron could not initialize its GPU process. Reruns outside the sandbox passed, with a fresh native result file. No manual UI testing or physical listening claimed. git diff --check passed.

Delivery: note-drag-paste-patch.zip with changed source, generated modules, tests and this log. No outstanding task-specific work.
