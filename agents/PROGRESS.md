...
## Download completes but Windows update installer never runs

**2026-09-09.** Investigated the reported download/close/no-update failure. The local userData updates directory contains the completed 175,378,039-byte 0.3.7 ZIP and generated helper script, with no extraction/error log; the E: installation remains 0.3.6. Public latest release metadata reports tag `0.3.7`, the expected asset and a populated SHA-256 digest. Both `0.3.7` and `v0.3.7` tags are accepted; tagging is not the failure. Read-only inspection did not execute the user's helper or modify that installation.

An isolated Electron probe reproduced Windows PowerShell exiting without executing a marker command under the old detached flags. Simply removing the detached flag was insufficient for the complete install/exit flow. The final fix in `updater.cjs` uses a short-lived hidden PowerShell launcher and `Start-Process -WindowStyle Hidden` for the independent installer; `main.cjs` awaits launcher completion before quitting. Startup output/error logs and a launch-error dialog replace silent bootstrap failures. The helper's existing validated extraction, copy, example preservation and restart behavior remains. Updated DEVELOPMENT/PROJECT_MAP and added `tests/electron-updater.cjs`. User-committed 0.3.7 files and unrelated generated working changes were preserved; no version bump, package, publication or installed-app replacement was performed. Existing affected installations need a one-time manual update to a future release containing this fix.

Validation: authorized `node tests/run.cjs` completed incremental build and **165/165** pure/model, CPU-audio and simulated renderer tests. Authorized `node tests/electron-updater.cjs` passed with fresh `.validation/electron-updater.json`: real Electron exit, independent Windows PowerShell extraction and file replacement, a silent fixture EXE restart, existing Example Project preservation, and archive/script cleanup. Intermediate launch variants failed this native fixture before the final bootstrap/await correction. Authorized `node node_modules/electron/cli.js tests/electron-close.cjs --case=save` passed with fresh evidence; its EISDIR console output is the intentionally simulated save failure. Native handoff uses a locally generated tiny ZIP/EXE, not the public 175 MB download or the user's installation. `git diff --check` passed.

## Lazy Ensemble: current character limit and ten-player ceiling

**2026-09-09.** Lazy Ensemble now reads the current Character limit at export and applies it directly per player, including 5,000 or 50,000. The ten-channel cap per player remains; output is rejected before any destination/save call if the smaller verified packing still needs more than ten players. More than 100 overlapping voices fails immediately; other cases try both packing orders and fitting merges before the player-count check. Error messages use the chosen limit. Updated `src/music/lazy-ensemble.ts`, `src/export.ts`, dialog text in `index.html`, generated outputs, existing sheet/native export tests and IMPORT_EXPORT/PROJECT_MAP. This supersedes earlier fixed-10,000 Lazy Ensemble documentation. Compression, synchronization, source notes and version-2 data remain unchanged; unrelated working changes preserved.

Validation: authorized `node tests/run.cjs` completed incremental build and **165/165** pure/model, CPU-audio and simulated renderer tests. New tests verify 5,000 versus 50,000 packing with identical decoded music, an allowed single file above 10,000 characters, exactly ten players, rejection of 101 overlapping notes and character-pressure-only overflow, and invalid limits. Authorized `node node_modules/electron/cli.js tests/electron-export-formats.cjs` passed; fresh `.validation/electron-export-formats.json` records a 5,000-character hint, successful export and rejection above ten players with no destination calls and a re-enabled Export button. Native programmatic UI/IPC uses stubbed OS destinations; no manual picker or in-game testing. `git diff --check` passed. No release was packaged.

## Remove superseded zero-time tempos in Extreme Compression

**2026-09-09.** Extreme Compression now removes an earlier T when another T replaces it before any note/rest advances time, including across O/V/L and tie prefixes. The supplied 60-second example now starts `o4t32` instead of `t120o4t32`. Tempos governing durations and unsuperseded final restorations remain. Cleanup belongs to `src/music/mml-optimizer.ts`, invoked only by Extreme Compression in `src/music/mml.ts`; ordinary editor output and version-2 data are unchanged. Updated generated outputs, existing sheet tests and IMPORT_EXPORT; ownership remains within the mapped compiler/optimizer modules.

Validation: authorized `node tests/run.cjs` completed the incremental build and **164/164** pure/model, CPU-audio and simulated renderer tests. Added checks cover zero-time O/V/L commands, tied continuation boundaries, retained tempos across rests/notes, the exact corrected output and decoded 60-second duration. `git diff --check` passed. No new native UI or in-game test for this pure export-text change; previous native export validation remains recorded below. Unrelated working changes preserved.

## Experimental Extreme Compression at export

**2026-09-09.** Added an unchecked Export > Options > Experimental Extreme Compression setting for MS2MML, Lazy Ensemble and text files. Exact local tempo/length substitutions shorten held notes and rests where tempo-command overhead still saves characters. Continuation ties follow tempo commands, note onsets/releases and V remain aligned, and a final whole-channel size comparison retains ordinary encoding unless the result shrinks. Editor notes, tempo markers, playback, MML windows, displayed character counts, overlays and version-2 JSON stay ordinary. Internal export planners count the actual compressed strings for limits. MIDI/audio hide and ignore the setting.

Changed `src/music/mml.ts` (bounded candidate search and loop option propagation), `src/music/sheets.ts`, `src/music/lazy-ensemble.ts`, `src/export.ts`, `index.html`, generated outputs, existing sheets/native export tests and IMPORT_EXPORT/PROJECT_MAP. The option searches conventional-length-derived integer tempos plus T32, not every possible encoding. Independent local clocks remain experimental and not a lossless importer round trip. Previous Lazy Ensemble and unrelated working changes are preserved; no release packaging/version change.

Validation: authorized `node tests/run.cjs` completed the incremental build and **163/163** pure/model, CPU-audio and simulated renderer tests. The initial run passed 162/163; the new common-end assertion incorrectly used an earlier-finishing channel instead of the maximum channel end and was corrected. New tests decode elapsed note onsets/releases and common ends across tempo/speed boundaries, held ties, V0, loops, sequential parts and Lazy Ensemble; already compact output and ordinary editor counts remain unchanged. Authorized `node node_modules/electron/cli.js tests/electron-export-formats.cjs` passed with fresh `.validation/electron-export-formats.json`: opt-in checkbox, format visibility, real MS2MML save IPC, unchanged source/editor MML and **260 → 86** raw saved characters for the long-note fixture. Native programmatic UI uses stubbed OS destinations; no manual picker, physical listening or in-game playback was tested. `git diff --check` passed with line-ending notices. Working folder updated directly.

## Lazy Ensemble MS2MML export

**2026-09-09.** Added Export > Lazy Ensemble (MS2MML). As confirmed by the user, selected music combines into one sound. All files/channels start at zero and retain full notes, resolved original instrument volumes and a shared elapsed-time end. Loops and speed zones are expanded/simulated; optional exact-rational T32 rests restore musical tempo before sound resumes. Each file is verified at no more than 10 channels and 10,000 raw characters. Section splitting and the ordinary configurable character limit do not apply. Source notes, version-2 data and unrelated working changes are preserved.

Changed owners: new `src/music/lazy-ensemble.ts`, opt-in rest compression in `src/music/mml.ts`, export integration in `src/export.ts` / `index.html`, generated modules, existing sheet and native export tests, IMPORT_EXPORT and PROJECT_MAP. Two packing orders and fitting-union merging seek fewer players but do not guarantee the mathematical minimum. Complete notes are never divided; an individually unrepresentable note plus silence fails before saving. Target fine-denominator warnings remain, and importing local-clock output is not a lossless round trip.

Validation: sandboxed `node build.cjs` compiled editor modules but failed on esbuild dependency directory access. Authorized `node tests/run.cjs` completed the incremental build and **161/161** pure/model, CPU-audio and simulated renderer tests. New sheet cases compare decoded elapsed onsets/releases, per-instrument V, 21 simultaneous notes requiring three players, selected scope, long rests with changing BPM, 2,600-note character overflow, loops/decimal speed and unchanged input JSON. After adding explicit common-end assertions, `node --test tests/sheets.test.mjs` passed **9/9**. Authorized `node node_modules/electron/cli.js tests/electron-export-formats.cjs` passed; fresh `.validation/electron-export-formats.json` records two real folder-export files containing 11 channels, preserved leading silence and both limits, plus existing format/color checks. This is native Electron programmatic UI/IPC with stubbed OS destinations, not manual file-picker or in-game playback. No portable release was packaged. Working folder updated directly.

## Build failure visibility and running-release guard

**2026-09-09.** Following the report that the build script no longer runs, executing `build.bat` confirmed that it launches and incremental compilation succeeds. Processes still run from the target repository release folder. Added an early process-path check in `package-release.ps1` to refuse replacement before clearing any generated files, with a specific close-the-copy message. `build.bat` now pauses on failure so double-clicked error output stays visible; automation uses `--no-pause`. Updated DEVELOPMENT. The user's original error text has been requested; the exact original failure has not been reproduced independently of the new guard.

Validation: authorized `build.bat` and `build.bat --no-pause` both compiled successfully and exited 1 at the new running-release guard. Focused updater tests passed 6/6 (simulated network/handoff checks); staging-only packaging succeeded with 103 files; `git diff --check` passed. No new native UI test for these build-script changes. The previous staged native startup test remains recorded below. Full release/ZIP creation and final release tests still await closing the running target copy; no processes were terminated or updates published.

## Missing native ZIP module in portable builds

**2026-09-09.** The reported startup crash is a packaging omission: `main.cjs` imports the export archive helper `zip.cjs`, which was absent from the explicit runtime file list. Added it in `package-release.ps1`, bumped package/lock versions to 0.3.6, and extended `tests/release.test.cjs` to resolve literal relative CommonJS imports in staging and release payloads. Updated DEVELOPMENT packaging guidance. Updater logic, musical behavior, version-2 storage and pre-existing generated working changes are preserved; ownership map remains accurate.

Validation: the new release regression reproduced the missing `./zip.cjs` in the old staging payload (the old manifest also correctly failed the new version comparison). Sandboxed `node build.cjs` hit the documented esbuild directory-access failure; authorized `node tests/run.cjs` then passed the incremental build and **157/157** pure/model, CPU-audio and simulated tests, including six updater tests. `powershell -NoProfile -ExecutionPolicy Bypass -File package-release.ps1 -StageOnly` staged 103 files successfully. An authorized temporary copy of the existing native `tests/electron-release.cjs` harness, changing only its main-module target to staging and evidence filename, passed startup, programmatic title changes, SoundFont IPC and synth initialization; fresh `.validation/electron-stage.json` reports success. This uses installed Electron with staged assets, not the rebuilt portable EXE or a real updater download/replacement. `git diff --check` passed with line-ending notices.

Delivery pending: the old release executable is still running from this repository's release folder. The user has been asked to close it before replacing generated release files. Staging and source are corrected; full portable folder/ZIP rebuild and final release-content/native-release tests remain pending. No release was published.
