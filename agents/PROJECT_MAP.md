...
Sound-bank discovery and validated asset IDs: [sound-banks.cjs](../sound-banks.cjs), main/preload IPC. Selection and preference live in [playback/transport.ts](../src/playback/transport.ts); stack loading/lifecycle in [playback/engine.ts](../src/playback/engine.ts), with per-program pitch policy in [sample-pitch.ts](../src/playback/sample-pitch.ts). Regressions: [sound-banks.test.mjs](../tests/sound-banks.test.mjs), [electron-sound-banks.cjs](../tests/electron-sound-banks.cjs). Maplebeats exclusions are shared by the renderer bundle and offline renderer through [audio/sound-bank.ts](../src/audio/sound-bank.ts); [audio/synth-entry.ts](../src/audio/synth-entry.ts) exposes the bank preparation helper in the synth bundle. The native harness needs assets/maplebeats-2.dls; the normal suite uses a generated DLS fixture if it is absent. Run the export harness with MML_STUDIO_TEST_BANK=maplebeats-2.dls to check the selected stack through the encoder.
