...
Sound-bank discovery and validated asset IDs: [sound-banks.cjs](../sound-banks.cjs), main/preload IPC. Selection and preference live in [playback/transport.ts](../src/playback/transport.ts); stack loading/lifecycle in [playback/engine.ts](../src/playback/engine.ts), with per-program pitch policy in [sample-pitch.ts](../src/playback/sample-pitch.ts). Regressions: [sound-banks.test.mjs](../tests/sound-banks.test.mjs), [electron-sound-banks.cjs](../tests/electron-sound-banks.cjs). Maplebeats dedicated drum remapping is shared by the renderer bundle and offline renderer through [audio/sound-bank.ts](../src/audio/sound-bank.ts); [audio/synth-entry.ts](../src/audio/synth-entry.ts) exposes the bank preparation helper in the synth bundle. The native harness needs assets/maplebeats-2.dls; the normal suite uses a generated DLS fixture if it is absent. Run the export harness with MML_STUDIO_TEST_BANK=maplebeats-2.dls to check the selected stack through the encoder.
...
| Multi-file Import and Add parts to this project | picker in `main.cjs` sorts by name; [files.ts](../src/files.ts) chooses replacement or addition and hands both to [drop-files.ts](../src/drop-files.ts) | [electron-import-menu](../tests/electron-import-menu.cjs), [electron-midi-import](../tests/electron-midi-import.cjs) |
...
| Feature | Primary code | Existing tests |
| --- | --- | --- |
| Draw both ways, Spray, select/move/resize with deferred move commits and viewport preview, right-button erase, box/move edge scrolling, pointer cancellation, ruler seek and key glide | [pointer.ts](../src/pointer.ts); hit testing/selection coordinates in [geometry.ts](../src/geometry.ts); shared timed acceleration in [edge-scroll.ts](../src/edge-scroll.ts) | [renderer](../tests/renderer.test.cjs), [electron-pitch-layout](../tests/electron-pitch-layout.cjs), [electron-smoke](../tests/electron-smoke.cjs) |
...
| Metronome click on every beat, accented on the downbeat | [playback/metronome.ts](../src/playback/metronome.ts) builds the beat list from [music/structure.ts](../src/music/structure.ts) and schedules against the audio clock; controls and clock in [playback/transport.ts](../src/playback/transport.ts) | [metronome](../tests/metronome.test.mjs), [electron-metronome](../tests/electron-metronome.cjs) |
| Tools screen commands on selected notes or the active instrument | [tools.ts](../src/tools.ts); pure conversion in [music/simplify-timing.ts](../src/music/simplify-timing.ts), [music/remove-overlap.ts](../src/music/remove-overlap.ts), [music/simplify-polyphony.ts](../src/music/simplify-polyphony.ts), [music/scale.ts](../src/music/scale.ts) | [simplify-timing](../tests/simplify-timing.test.mjs), [remove-overlap](../tests/remove-overlap.test.mjs), [renderer](../tests/renderer.test.cjs), [electron-simplify-timing](../tests/electron-simplify-timing.cjs), [simplify-polyphony](../tests/simplify-polyphony.test.mjs), [electron-tool-simplifiers](../tests/electron-tool-simplifiers.cjs), [electron-ui](../tests/electron-ui.cjs) |
...
| Note fill/borders/labels/handles, complementary sustained-overlap rails, single-selection top layering, selection outlines/box, offscreen culling | [rendering/notes.ts](../src/rendering/notes.ts), [music/note-visibility.ts](../src/music/note-visibility.ts) | [note-visibility](../tests/note-visibility.test.mjs), [renderer](../tests/renderer.test.cjs) |
| Shared event/warning caption wrapping, stacking and painting | [rendering/captions.ts](../src/rendering/captions.ts); entries owned by `rendering/instructions.ts` and `rendering/note-density.ts` | [renderer](../tests/renderer.test.cjs), [electron-mml](../tests/electron-mml.cjs) |
| Immediate-jump horizontal scrollbar, thumb drag and keyboard navigation | [horizontal-scroll.ts](../src/horizontal-scroll.ts), installed by renderer and refreshed by painting; `index.html` / `studio.css` | [renderer](../tests/renderer.test.cjs), [electron-structure](../tests/electron-structure.cjs), [electron-pitch-layout](../tests/electron-pitch-layout.cjs) |
| Horizontal scrollbar section/song landmarks and hover names | [scroll-markers.ts](../src/scroll-markers.ts), called from painting; `index.html` / `studio.css` | [renderer](../tests/renderer.test.cjs), [electron-structure](../tests/electron-structure.cjs) |
...
| Instruction bands, clickable stacked captions, loop shading and blue default tempo indicators | [rendering/instructions.ts](../src/rendering/instructions.ts); [rendering/tempo.ts](../src/rendering/tempo.ts) is a compatibility re-export | [renderer](../tests/renderer.test.cjs), [electron-timeline](../tests/electron-timeline.cjs), [electron-loops](../tests/electron-loops.cjs) |
| Active-instrument yellow density regions/ruler markers and persistent orange overlap onset lines with shared warning captions | [rendering/note-density.ts](../src/rendering/note-density.ts), pure overlap locations in [music/note-density.ts](../src/music/note-density.ts) | [note-density](../tests/note-density.test.mjs), [renderer](../tests/renderer.test.cjs), [electron-mml](../tests/electron-mml.cjs) |
...
| Identical-onset/pitch/instrument warning, sustained same-pitch overlap spans and >10 sounding-note sweep | [music/note-density.ts](../src/music/note-density.ts); warning used by MML and MIDI import | [note-density](../tests/note-density.test.mjs), [midi-import](../tests/midi-import.test.mjs) |
...
| Simulated speed zone pairing, nested factors, inherited view context | [music/speed.ts](../src/music/speed.ts); consumed by tempo, loops, MML, sheets, projections/slices and instruction UI/rendering | [loops](../tests/loops.test.mjs), [mml](../tests/mml.test.mjs), [renderer](../tests/renderer.test.cjs), [electron-loops](../tests/electron-loops.cjs) |
...
| Opt-in imported tempo conversion to integer/power-of-two Speed Multipliers | [import/tempo.ts](../src/import/tempo.ts); shared prompt/report in [import-ui.ts](../src/import-ui.ts), called by file replacement and drop imports | [midi-import](../tests/midi-import.test.mjs), [electron-midi-import](../tests/electron-midi-import.cjs) |
| Keep/remove imported T on populated drops; non-blocking same-position tempo resolution | [import-ui.ts](../src/import-ui.ts) coordinates opt-in choice before speed conversion; [drop-files.ts](../src/drop-files.ts) selects additive versus empty-project replacement, sharing `replaceWithImport` in [files.ts](../src/files.ts); native named choice in main/preload; stripping in [import/tempo.ts](../src/import/tempo.ts), file-only MML policy in [import/source.ts](../src/import/source.ts) / [import/mml.ts](../src/import/mml.ts), existing-clock replacements in [import/append.ts](../src/import/append.ts) | [midi-import](../tests/midi-import.test.mjs), [mml-import](../tests/mml-import.test.mjs), [electron-midi-import](../tests/electron-midi-import.cjs) |
| Drop MIDI/MML/ABC/text files as new instruments | [drop-files.ts](../src/drop-files.ts) owns browser files, async guards, history/scope and commit; [import/source.ts](../src/import/source.ts) shares MIDI/MML/ABC dispatch; [import/append.ts](../src/import/append.ts) remaps new lanes/IDs | [midi-import](../tests/midi-import.test.mjs), [abc-import](../tests/abc-import.test.mjs), [renderer](../tests/renderer.test.cjs), [electron-midi-import](../tests/electron-midi-import.cjs) |
| Voice-line audio drops: bundled FFmpeg PCM decode, five spectral voices, 30 ms timing, velocity and character report | [audio-import.cjs](../audio-import.cjs), authenticated main/preload IPC; pure [import/audio.ts](../src/import/audio.ts), dispatched by [drop-files.ts](../src/drop-files.ts); runtime list in `package-release.ps1` | [audio-import](../tests/audio-import.test.mjs), [electron-audio-import](../tests/electron-audio-import.cjs) |
| Audio Sample Rate / Voice Count dialog, clip-length budget estimate and cancellation | [audio-import-ui.ts](../src/audio-import-ui.ts), `index.html` / `studio.css`; pure interval normalization and estimate in [import/audio.ts](../src/import/audio.ts); invoked after decode by `drop-files.ts` | [audio-import](../tests/audio-import.test.mjs), [electron-audio-import](../tests/electron-audio-import.cjs), [renderer](../tests/renderer.test.cjs) |
| Single-part ABC and game-exporter chord timing, headers, accidentals and ties | [import/abc.ts](../src/import/abc.ts); filename dispatch in `import/source.ts`, menu filter in `main.cjs` | [abc-import](../tests/abc-import.test.mjs), [electron-midi-import](../tests/electron-midi-import.cjs); optional `MML_ABC_FIXTURE` environment path exercises a user file through native DragEvent |
| Best-effort ABC file recovery, text encoding fallback, one silent instruction per timestamp | [import/abc.ts](../src/import/abc.ts) owns recovery; [import/source.ts](../src/import/source.ts) opts in and decodes text; [import/instructions.ts](../src/import/instructions.ts) condenses payloads/IDs, also called by speed conversion and additive merge | [abc-import](../tests/abc-import.test.mjs), [midi-import](../tests/midi-import.test.mjs), [electron-midi-import](../tests/electron-midi-import.cjs) |
...
| Lazy Ensemble one-sound packing, zero-origin synchronized files, ten-player/channel caps and current character limit | [music/lazy-ensemble.ts](../src/music/lazy-ensemble.ts); opt-in rational rest compression in [music/mml.ts](../src/music/mml.ts) | [sheets](../tests/sheets.test.mjs), [electron-export-formats](../tests/electron-export-formats.cjs) |
| Export-only Experimental Extreme Compression, exact local note/rest clocks and actual saved-size checks | [music/mml.ts](../src/music/mml.ts); options passed by [music/sheets.ts](../src/music/sheets.ts), [music/lazy-ensemble.ts](../src/music/lazy-ensemble.ts) and [export.ts](../src/export.ts) / `index.html` | [sheets](../tests/sheets.test.mjs), [electron-export-formats](../tests/electron-export-formats.cjs) |
| Export dialog, five choices, selected/all scope, section splitting, channel-overflow cancel/continue/fit, single/parts/cancel and audio progress | [export.ts](../src/export.ts), `index.html`, `studio.css`; MIDI compiler and audio worker below | [renderer](../tests/renderer.test.cjs), [electron-export-formats](../tests/electron-export-formats.cjs), [electron-sheets](../tests/electron-sheets.cjs), [electron-audio-export](../tests/electron-audio-export.cjs) |
...
| Bank selection, custom-first/GM fallback, lazy SoundFont/worklet synth, independent key-preview synth, shared master gain | [playback/engine.ts](../src/playback/engine.ts) | [playback](../tests/playback.test.mjs), [preview](../tests/preview.test.cjs), [electron-smoke](../tests/electron-smoke.cjs) |
...
| Transport, seek, live note/instruction/preset reload, Mute/Solo routes, speed/master/effective BPM | [playback/transport.ts](../src/playback/transport.ts) | [renderer](../tests/renderer.test.cjs), [electron-behavior](../tests/electron-behavior.cjs) |
...
| Public GitHub release check, verified ZIP download and portable PowerShell handoff | [updater.cjs](../updater.cjs), wired by [main.cjs](../main.cjs); packaged by [package-release.ps1](../package-release.ps1) | [updater](../tests/updater.test.cjs), native fixture handoff in [electron-updater](../tests/electron-updater.cjs), packaged payload in [release](../tests/release.test.cjs) |
...
| Three audio bundles with SF3 decoder disabled (SF2/DLS supported) | [build-audio.cjs](../build-audio.cjs), local rejection adapter [index.js](../packages/sf2-only-decoder/index.js) and [package.json](../packages/sf2-only-decoder/package.json). Generates `vendor/synth.js`, `vendor/spessasynth_processor.min.js`, `vendor/audio-worker.cjs`. |
