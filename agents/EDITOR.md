...
- **Simplify Timing**: choose L4/L8/L16/L32/L64. Each edge extends to include its grid window only when the note covers **more than 40%** of that window; smaller coverage is trimmed. New overlaps are prevented; existing polyphony remains. Notes that cannot fit retain their original timing and are reported. Rapid sequences (each note L32 or shorter and shorter than the chosen window) may repeat pitches, rise, fall or turn: where **two or more** consecutive starts share a window, retain its first note and fill that window. Thus `C32C32` becomes `C16`, and `C32D32E32F32` becomes `C16E16` at L16. Longer sequential notes competing in one window compare their coverage within that window: a 40/60 through 60/40 split (inclusive) becomes equal halves; otherwise the larger share takes the whole window. Portions held outside that window remain, and a losing note is removed only if no duration remains. Existing chords/sustained overlaps are excluded from competition. A hammer-on is an L32-or-shorter lead-in into a different-pitch note **more than six times longer**; remove the lead-in and keep the ending note, rounding it normally. Consecutive ornament notes may touch or have a one-unit gap. Simultaneous onsets are excluded from detection. Condensation protects attached instructions, unrelated notes and the current view end; surviving inherited velocities, including V0, are retained when carriers are removed. Conflicting instruction conversions are rejected and Undo restores all removed notes.
