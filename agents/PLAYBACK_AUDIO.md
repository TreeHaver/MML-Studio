...
The release builder requires and includes ms2.dls alongside TimGM6mb.sf2 so new installations receive the MS2 default. Other custom files remain local user assets and are not automatically included. The user-provided ms2.dls is not added to Git. SF3 remains unsupported; DLS is decoded natively by the pinned core and needs no conversion to SF2 or external decoder.
...
Changing presets and notes refreshes the complete snapshot at the current musical position while preserving playing/paused state. Rapid edits received during a load are coalesced into the latest snapshot; Stop cancels pending loads. Notes spanning the playhead are restored on their assigned routes with resolved onset V and remaining duration, while removed or moved-away notes stop sounding. Reloads briefly pause/re-attack sustained notes; this is not seamless audio mutation. The current loop occurrence is retained when its source mapping survives the edit; changed loop structure seeks the same source position in the new plan. Existing scoped/lane-count history operations that explicitly stop playback still do so.
