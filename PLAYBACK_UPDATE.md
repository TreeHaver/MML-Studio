...
## Full melodic pitch range — 2026-09-08

User confirmed all C0–B8 pitches must sound, including B-1/C9 reached through MS2 o0c-/o8b+. GUI labels use #; generated MML uses +/-. Keyboard and song preview now share src/playback/sample-pitch.ts. It records audible key zones in bundled TimGM6mb.sf2 (SHA256 c5378b62028c920cb11e4803327983fee2f2cdff5dc89c708e39da417e51c854), measured with synth effects disabled to exclude residual reverb. Most presets stop at MIDI108; several have low or interior gaps. Missing notes select an octave-equivalent sample in the same preset, tuned to the requested pitch. No sound-bank asset or project pitch is altered. Standard Drum Kit retains its defined percussion-key mapping; fixed MS2 drums remain mapped normally.

Song routes have fixed tuning for their entire lifetime, so changes of pitch do not retune release tails. Each tuning group uses the shared monophonic allocation; these additional temporary routes do not change MML voice counts. MIDI RPN sets wheel sensitivity before note-on. The helper compensates for the bundled SF2 modulator's 127/128 sensitivity scaling and whole-cent truncation. Coarse-tuning RPN was unsuitable because this synth uses it to select another sample. Held-note restoration applies the same sample and tuning; mute/solo and presets apply to every owning route. V0 remains silent by intent.

Validation: 128 presets × 110 pitches (MIDI11–120) × V1/V8/V15 = 42,240 actual dry-synth cases, all audible. Additional checks measure upward/downward octave frequency ratios, MIDI routes/controllers/note ends/velocities, seek restoration and unchanged source data. Native Electron AudioWorklet tests verify overlapping C#8 song notes through seeks, live/paused preset changes, plus audible keyboard preview and the C#8 GUI label. No physical-speaker or in-game check. Revalidate the range table and frequency tests when changing the sound bank or synth version.
