...
export function syncSegment(){
 ...
 state.segment={root,projection,sourceIndices:projection.project.instruments.map((_,index)=>index<root.instruments.length?index:-1)};
 state.project=projection.project;state.active=Math.max(0,Math.min(active,state.project.instruments.length-1));state.selectedInstruments=new Set(selectedInstruments.filter(index=>state.project.instruments[index]&&!state.project.instruments[index].isInstructions));
