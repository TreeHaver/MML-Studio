...
export function audioPlan(request:AudioRequest){
 ...
 for(const event of midi.events){
  seconds+=(event.tick-tick)*micros/1000000/midi.ppq/request.speed;tick=event.tick;
...
export async function renderAudio(request:AudioRequest,bankBytes:ArrayBuffer,write:(pcm:Float32Array)=>Promise<void>,progress:(fraction:number)=>void=()=>{},cancelled:()=>boolean=()=>false){
 ...
 try{
  while(frame<tailLimit){
   ...
   while(index<plan.events.length&&plan.events[index].frame<=frame){const event=plan.events[index++];synth.processMessage(event.message,event.offset);}
   ...
   for(let i=0;i<count;i++)for(const value of [left[i]*request.volume,right[i]*request.volume]){
    ...
    if(used===chunk.length){await write(chunk);used=0;}
   }
   frame+=count;
   ...
   if(percent!==lastProgress){lastProgress=percent;progress(percent/100);}
 ...
 }finally{synth.stopAllChannels(true);}
}
