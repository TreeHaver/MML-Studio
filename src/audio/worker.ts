...
import {renderAudio,SAMPLE_RATE} from './render.ts';
...
async function run(){
 ...
 let finished:Promise<void>|undefined,closed=false;
 try{
  ...
  finished=new Promise<void>((resolve,reject)=>{
   child!.on('error',reject);child!.on('close',code=>{closed=true;code===0?resolve():reject(Error(stderr||`Audio encoder exited with code ${code}.`));});
  });
  ...
  const result=await renderAudio(request,bytes.buffer.slice(bytes.byteOffset,bytes.byteOffset+bytes.byteLength),async pcm=>{
   ...
   await new Promise<void>((resolve,reject)=>child!.stdin!.write(Buffer.from(buffer),error=>error?reject(error):resolve()));
   await setImmediate();
 ...
 }catch(error){parentPort!.postMessage({error:cancelled?'Audio export canceled.':String(error),canceled:cancelled});}
 finally{
  if(child&&!closed)child.kill();
  await finished?.catch(()=>{});
  await unlink(temporary).catch(()=>{});parentPort!.close();
 }
}
void run();
