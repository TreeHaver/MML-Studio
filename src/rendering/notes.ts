...
import {sustainedOverlapSpans} from '../music/note-density.ts';
...
let sustainedSpans:ReturnType<typeof sustainedOverlapSpans>=[];
...
export function overlapOutlineColor(background:string){
 let color=overlapColors.get(background);
 if(!color){const {h,s,v}=hexToHsv(background);color=hsvToHex((h+180)%360,s,v);overlapColors.set(background,color);}
 return color;
}
...
export function drawNotes(){
 ...
 if(state.selection.size===1){const index=visible.findIndex(n=>state.selection.has(n.id));if(index>=0)visible.push(...visible.splice(index,1));}
 ...
 for(const span of state.gesture?.movePreview?[]:sustainedSpans){
  if(isMuted(span.instrument)||!instrumentSelected(span.instrument))continue;
  const r=rect({...span,id:0,length:span.end-span.start,volume:null});
  const thickness=2,gap=1;
  if(r.x+r.w<=KEY||r.x>=state.width||r.y+r.h+gap+thickness<=HEAD||r.y-gap-thickness>=state.height)continue;
  ctx.fillStyle=overlapOutlineColor(state.project.instruments[span.instrument].color);
  ctx.fillRect(r.x,r.y-gap-thickness,r.w,thickness);ctx.fillRect(r.x,r.y+r.h+gap,r.w,thickness);
 }
