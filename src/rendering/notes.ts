...
export function drawNotes(){
 ...
 ctx.save();ctx.beginPath();ctx.rect(r.x+2,r.y,Math.max(0,r.w-4),r.h);ctx.clip();ctx.fillStyle='#161b20';ctx.font='10px Segoe UI, sans-serif';ctx.textBaseline='middle';ctx.fillText(instructions?(n.tempo==null?'Event':`T${n.tempo}`):name(n.pitch)+(n.tempo==null?'':` T${n.tempo}`),r.x+4,r.y+r.h/2);ctx.restore();ctx.globalAlpha=1;
 }
