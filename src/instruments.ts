...
import {state,instrumentView,isMuted} from './state.ts';
...
export function instruments(){
 ...
 const mute=document.createElement('button');mute.textContent='Mute';mute.setAttribute('aria-pressed',String(isMuted(index)));mute.classList.toggle('active',isMuted(index));
 mute.onclick=()=>{instrumentView.solo=null;if(instrumentView.muted.has(index))instrumentView.muted.delete(index);else instrumentView.muted.add(index);changed();};
 const solo=document.createElement('button');solo.textContent='Solo';solo.setAttribute('aria-pressed',String(instrumentView.solo===index));solo.classList.toggle('active',instrumentView.solo===index);
