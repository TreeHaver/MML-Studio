...
export function installAppearance(){
 ...
 const decorateSelect=(select:HTMLSelectElement)=>{
  ...
  const close=()=>{panel?.remove();panel=null;shell.classList.remove('open');if(closeOpenList===close)closeOpenList=null;};
  const open=()=>{
   ...
   for(const option of [...select.options]){
    ...
    if(option.selected)current=item;
   ...
   if(height<full&&current)panel.scrollTop=Math.max(0,current.offsetTop-height/2);
  };
  select.addEventListener('pointerdown',e=>{e.preventDefault();open();});
  select.addEventListener('keydown',e=>{if(e.key==='Escape')close();else if(e.key==='Enter'||e.key===' '||(e.key==='ArrowDown'&&e.altKey)){e.preventDefault();open();}});
 };
