...
export function installAppearance(){
 ...
 const decorateSelect=(select:HTMLSelectElement)=>{
  ...
  const fill=()=>{(select as any).fillOptions?.();};
  ...
  const highlight=(index:number)=>{
   ...
   const item=items[index];item.classList.add('current');
   if(item.offsetTop<panel.scrollTop)panel.scrollTop=item.offsetTop-4;
  ...
  const typeAhead=(character:string)=>{
   window.clearTimeout(typedTimer);typedTimer=window.setTimeout(()=>{typed='';},900);
   ...
   if(found<0&&typed.length>1){typed=character.toLowerCase();found=options.findIndex(option=>label(option).startsWith(typed));}
   if(found<0)found=options.findIndex(option=>label(option).includes(typed));
   highlight(found);
  };
  const open=()=>{
   if(panel){close();return;}
   closeOpenList?.();
   panel=document.createElement('div');panel.className='menu-panel select-panel';panel.onpointerdown=e=>e.preventDefault();
   items=visibleOptions().map(option=>{
    const item=document.createElement('button');item.type='button';item.textContent=option.textContent;item.disabled=option.disabled;if(option.title)item.title=option.title;
   ...
   items[chosen]?.classList.add('current');active=chosen;
   if(height<full&&items[chosen])panel.scrollTop=Math.max(0,items[chosen].offsetTop-height/2);
   select.focus({preventScroll:true});
  };
  select.addEventListener('pointerdown',e=>{e.preventDefault();open();});
  select.addEventListener('keydown',e=>{
   if(e.key==='Escape'){close();return;}
   if(!panel){if(e.key==='Enter'||e.key===' '||(e.key==='ArrowDown'&&e.altKey)){e.preventDefault();open();}return;}
   if(e.key==='ArrowDown'||e.key==='ArrowUp'){e.preventDefault();highlight(Math.max(0,Math.min(items.length-1,active+(e.key==='ArrowDown'?1:-1))));return;}
   if(e.key==='Home'||e.key==='End'){e.preventDefault();highlight(e.key==='Home'?0:items.length-1);return;}
   if(e.key==='Enter'){e.preventDefault();items[active]?.click();return;}
 ...
 try{const saved=JSON.parse(localStorage.getItem(key)??'null');if(saved){if(['sky','night'].includes(saved.theme))settings.theme=saved.theme;for(const side of ['left','right'] as const){if(Number.isFinite(saved[side]))settings[side]=Math.max(180,Math.min(480,saved[side]));}settings.leftHidden=saved.leftHidden===true;settings.rightHidden=saved.rightHidden===true;settings.piano=saved.piano===true;}}catch{}
 ...
 const theme=()=>{
  ...
  document.querySelectorAll('#theme-menu .theme-option').forEach(btn=>btn.setAttribute('aria-current',String((btn as HTMLElement).dataset.theme===settings.theme)));
  draw();
 };
 document.querySelectorAll('#theme-menu .theme-option').forEach(btn=>{(btn as HTMLElement).onclick=()=>{settings.theme=(btn as HTMLElement).dataset.theme!;theme();save();};});
 ...
 const keys=()=>{
  ...
  const button=$('key-style');button.setAttribute('aria-pressed',String(settings.piano));
 ...
 const playback=$('playback-menu') as HTMLDetailsElement;
 const playbackPanel=playback.querySelector('.playback-panel') as HTMLElement;
 const placePlayback=()=>{
  ...
  if(over>0&&room>0)playbackPanel.style.left=`${-Math.min(over,room)}px`;
 };
 ...
 theme();keys();layout();new ResizeObserver(layout).observe(main);
}
