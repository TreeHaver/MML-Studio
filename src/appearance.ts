...
export function installAppearance(){
 ...
 try{const saved=JSON.parse(localStorage.getItem(key)??'null');if(saved){if(['sky','night'].includes(saved.theme))settings.theme=saved.theme;for(const side of ['left','right'] as const){if(Number.isFinite(saved[side]))settings[side]=Math.max(180,Math.min(480,saved[side]));}settings.leftHidden=saved.leftHidden===true;settings.rightHidden=saved.rightHidden===true;}}catch{}
