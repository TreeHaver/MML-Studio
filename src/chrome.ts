...
export function installChrome(){
 ...
 const menus=[$('file-menu'),$('theme-menu'),$('section-menu'),$('playback-menu'),$('instrument-options')] as HTMLDetailsElement[];
 document.onclick=event=>{
  ...
  for(const menu of menus)if(!menu.contains(target)||button&&!stays.includes(button.id))menu.open=false;
 };
