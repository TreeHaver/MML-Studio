...
let previousNotes:typeof state.project.notes|undefined,previousCount=-1,previousRoles='';
let markers:ReturnType<typeof sectionMarkers>=[],previousLayout='';
...
export function refreshScrollMarkers(){
 ...
 if(notes!==previousNotes||notes.length!==previousCount||roles!==previousRoles){
  markers=sectionMarkers(state.project);previousNotes=notes;previousCount=notes.length;previousRoles=roles;previousLayout='';
 }
 ...
 if(key===previousLayout)return;previousLayout=key;
 strip.hidden=!markers.length;strip.style.width=width+'px';strip.style.bottom=bottom+'px';
 strip.replaceChildren();
 for(const marker of markers){
  const tick=document.createElement('span');tick.className='scroll-marker'+(marker.song?' song':'');
  const label=`${marker.song?'Song':'Section'}: ${marker.name}`;
  tick.title=label;tick.setAttribute('aria-label',label);tick.setAttribute('role','img');
