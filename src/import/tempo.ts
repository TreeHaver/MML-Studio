...
export function withoutImportedTempos(project:Project):Project{
 ...
 for(const note of result.notes)delete note.tempo;
 return result;
}

function targetTempo(bpm:number){
 if(!outsideTarget(bpm))return {base:bpm,multiplier:1};
 if(bpm<32){const multiplier=bpm>=16?0.5:bpm>=8?0.25:1;return {base:bpm/multiplier,multiplier};}
...
export function applyImportSpeedMultipliers(project:Project,warnings:string[]=[]):Project{
 if(!hasOutOfRangeTempo(project))return project;
 ...
 for(const [bpm,{base,multiplier}] of settings){
  if(outsideTarget(base))warnings.push(`Tempo ${bpm} BPM was left unchanged: it cannot fit T32–T255 using ×2/×4 (within 2 BPM), or exact ÷2/÷4 for slow tempos.`);
  else if(base*multiplier!==bpm)warnings.push(`Tempo ${bpm} BPM was approximated as ${base*multiplier} BPM (T${base} ×${multiplier}; error ${Math.abs(base*multiplier-bpm)} BPM).`);
 }
 if([...settings.values()].every(s=>s.multiplier===1))return project;
 ...
 for(const note of converted.notes)if(note.tempo!=null)note.tempo=settings.get(note.tempo)!.base;
 for(const {tick,bpm} of clock){
  const {multiplier}=settings.get(bpm)!;
  if(multiplier===previous)continue;
