...
import {projectVolumes,shiftProjectVolumes,MAX_VOLUME} from './model/instrument-operations.ts';
 ...
 */
export function shiftAllVolumes(delta:number){
 const range=projectVolumes(state.project);
 if(!range){status('There are no notes to change.');return;}
 if(!delta){status('Type how much to move the volume by. Negative numbers lower it.');return;}
 if(delta>0&&range.headroom<=0){status(`“${state.project.instruments[range.loudestInstrument].name}” is already at V${MAX_VOLUME}. Raising further would flatten it against the cap.`);return;}
 if(delta<0&&range.min<=0){status(`“${state.project.instruments[range.quietestInstrument].name}” is already at V0. Lowering further would flatten it against silence.`);return;}
 const step=delta>0?Math.min(delta,range.headroom):Math.max(delta,-range.min);
 checkpoint();state.project={...state.project,notes:shiftProjectVolumes(state.project,step)};refresh();
...
export function installTools(){
 ...
 $('volume-max').onclick=()=>{
  const range=projectVolumes(state.project);
  if(!range){status('There are no notes to change.');return;}
  if(range.headroom<=0){status(`“${state.project.instruments[range.loudestInstrument].name}” is already at V${MAX_VOLUME}: there is no room left.`);return;}
  shiftAllVolumes(range.headroom);volumeReading();
 };
