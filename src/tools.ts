...
import {simplifyHeldNotes,simplifyChords} from './music/simplify-polyphony.ts';
import {resolveVolumes} from './music/volume.ts';
import type {Note,Project} from './model/types.ts';
import {detectScale} from './music/scale.ts';
...
export function toolTargets(){
 ...
 stack.delete(state.active);stack.add(state.active);
 ...
 for(const note of notes){const group=groups.get(note.instrument)??[];group.push(note);groups.set(note.instrument,group);}
...
function volumeRange(){
 ...
 let min=15,max=0;for(const n of notes){const v=volumes.get(n.id)!;min=Math.min(min,v);max=Math.max(max,v);}
 return {min,max,headroom:15-max};
}
export function shiftToolVolumes(delta:number){
 const range=volumeRange();
 if(!range){status('Select musical notes or a musical instrument.');return;}
 const step=delta,clamped=range.min+delta<0||range.max+delta>15;
 if(!step){status('Volume unchanged: choose a nonzero amount.');return;}
 ...
 if(notes.some((n,i)=>n.volume!==state.project.notes[i].volume))commitNotes(notes);status('Volume: target notes moved by '+(step>0?'+':'')+step+'.'+(clamped?' Warning: volumes were clamped to V0–V15; velocity differences may be flattened.':''));
}
function applyTool(label:string,convert:(project:Project,index:number,ids:ReadonlySet<number>)=>{notes:Note[],changed:number,[key:string]:unknown},describe:(result:Record<string,number>)=>string){
 const selected=toolTargets(),ids=new Set(selected.map(n=>n.id));
 if(!ids.size){status('Select musical notes or a musical instrument.');return;}
 let project=state.project;const totals:Record<string,number>={changed:0};
 for(const index of new Set(selected.map(n=>n.instrument))){
  const result=convert(project,index,ids);project={...project,notes:result.notes};
  for(const [key,value] of Object.entries(result))if(typeof value==='number')totals[key]=(totals[key]??0)+value;
 }
 if(!valid(project.notes)){status(label+': unchanged because the edit would conflict with instructions.');return;}
 if(totals.changed)commitNotes(project.notes);
...
export function installTools(){
 ...
 const dialog=$('tools-dialog') as HTMLDialogElement;
 $('tools-open').onclick=()=>{volumeReading();$('tools-status').textContent='';if(!dialog.open)dialog.showModal();};
 ...
 $('volume-max').onclick=()=>{const range=volumeRange();if(range)shiftToolVolumes(range.headroom);else status('No musical target notes.');volumeReading();};
 ...
 input('chord-complexity').value='2';input('chord-use-scale').checked=true;
 $('simplify-chords').onclick=()=>{
  const complexity=Number(input('chord-complexity').value);
  if(!Number.isSafeInteger(complexity)||complexity<2){status('Chord Complexity must be a whole number of at least 2.');return;}
