...
export function createEdgeScroll(){
 ...
 return {
  reset(){previous=undefined;for(const axis of axes)axis.direction=0;},
  step(point:Point,bounds:Bounds,vertical:boolean,time=previous===undefined?0:previous+1000/60){
   const elapsed=previous===undefined?1000/60:Math.max(0,Math.min(50,time-previous));previous=time;
   const axis=(index:number,value:number,low:number,high:number,enabled=true)=>{
    const near=value-low,far=high-value;
    const direction=!enabled?0:near<EDGE_SCROLL.zone&&near<=far?-1:far<EDGE_SCROLL.zone?1:0;
    const state=axes[index];if(direction!==state.direction){state.direction=direction;state.since=time;}
