...
export function snapTick(tick:number){
...
export const freeTick=(tick:number)=>Math.max(0,Math.round(tick));
/**
 ...
 */
export function setLoopRegion(a:number,b:number,grid=false){
 const place=grid?snapTick:freeTick;
 const start=place(Math.min(a,b)),end=place(Math.max(a,b));
 if(end<=start){clearLoopRegion();return false;}
 loopRegion.start=start;loopRegion.end=end;loopRegion.on=true;return true;
}
 ...
 */
export function markLoopStart(tick:number,musicEnd=0,grid=false){
...
export function markLoopEnd(tick:number,grid=false){
...
export function alignLoopToGrid(){
 ...
 loopRegion.on=placed&&on;return placed;
}
/** Keeps the region while it is switched off, so it can be brought back unchanged. */
export function toggleLoop(){if(loopSpan()>0)loopRegion.on=!loopRegion.on;return looping();}
/** Which end the pointer is on, so an existing loop can be dragged wider or narrower. */
export function loopEdgeAt(tick:number,tolerance:number){
 if(loopSpan()<=0)return null;
 const toStart=Math.abs(tick-loopRegion.start),toEnd=Math.abs(tick-loopRegion.end);
 if(Math.min(toStart,toEnd)>tolerance)return null;
