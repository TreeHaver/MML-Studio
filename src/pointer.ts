...
canvas.onpointerdown=e=>{
 ...
 if(e.shiftKey||(!n&&state.tool==='select'))state.gesture={kind:'box',start:p,current:p,music:m,add,before};
  ...
  if(!valid([...state.project.notes,newNote]))return;state.project.notes.push(newNote);state.selection=new Set([newNote.id]);state.gesture={kind:instructions?'instruction-create':'paint',start:p,current:p,music:m,nid:newNote.id,before,...(instructions?{}:{painted:new Set([`${start}:${m.pitch}`])})};
 }
 canvas.setPointerCapture(e.pointerId);info();draw();
};
canvas.onpointermove=e=>{
 const p=point(e);if(keyGesture){if(p.x>=0&&p.x<KEY&&p.y>=HEAD){const pitch=musical(p).pitch;if(pitch!==keyGesture.pitch&&pitch>=0&&pitch<=127){keyGesture.pitch=pitch;previewKey(p);}}return;}if(!state.gesture){const n=p.x>=KEY&&p.y>=HEAD?hit(p):undefined;canvas.style.cursor=n&&edge(n,p)?'ew-resize':'default';return;}
...
canvas.onpointerup=e=>{if(keyGesture){keyGesture=null;if(canvas.hasPointerCapture(e.pointerId))canvas.releasePointerCapture(e.pointerId);return;}if(!state.gesture)return;
...
canvas.onpointercancel=()=>{keyGesture=null;endGesture(true);};canvas.onlostpointercapture=()=>{keyGesture=null;if(state.gesture)endGesture(true);};canvas.oncontextmenu=e=>e.preventDefault();
