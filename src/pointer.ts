...
import {snap} from './music/timing.ts';
...
canvas.onpointerdown=e=>{
 if(e.button!==0&&e.button!==2)return;const p=point(e);if(p.y<HEAD)return;if(p.x<KEY){
  if(e.button===0&&p.x>=0&&!isMuted(state.active)){e.preventDefault();canvas.focus();const instrument=state.project.instruments[state.active];if(instrument.isInstructions){status('Instructions are silent. Draw a marker in the roll and set its tempo.');return;}void previewNote(musical(p).pitch,instrument.midiProgram??0,instrument.isDrum===true);}
  return;
 }
 ...
 if(e.shiftKey||(!n&&state.tool==='select'))state.gesture={kind:'box',start:p,current:p,music:m,add,before};
  ...
  const already=state.selection.has(n.id);if(!already){state.selection=new Set([n.id]);}
 ...
 }else{const start=Math.max(0,snap(m.tick,state.project.grid)),instructions=state.project.instruments[state.active].isInstructions;const newNote:Note={id:state.project.notes.reduce((id,n)=>Math.max(id,n.id),0)+1,instrument:state.active,start,length:instructions?1:128/state.project.grid,pitch:m.pitch,volume:instructions?0:null,...(instructions?{tempo:tempoAt(state.project.notes,start)}:{})};
  if(!valid([...state.project.notes,newNote]))return;state.project.notes.push(newNote);state.selection=new Set([newNote.id]);state.gesture={kind:instructions?'instruction-create':'create',start:p,current:p,nid:newNote.id,before};
 }
 canvas.setPointerCapture(e.pointerId);info();draw();
};
canvas.onpointermove=e=>{
 const p=point(e);if(!state.gesture){const n=p.x>=KEY&&p.y>=HEAD?hit(p):undefined;canvas.style.cursor=n&&edge(n,p)?'ew-resize':'default';return;}
 ...
 if(state.gesture.kind==='create'){const n=state.project.notes.find(n=>n.id===state.gesture.nid)!;const len=Math.max(128/state.project.grid,snap(musical(p).tick-n.start,state.project.grid));const next=state.project.notes.map(o=>o===n?{...n,length:len}:o);if(valid(next))state.project.notes=next;}
 info();draw();
};
canvas.onpointerup=e=>{if(!state.gesture)return;
 if(state.gesture.kind==='box'){const ids=boxIds(state.gesture.music,musical(state.gesture.current));state.selection=state.gesture.add?new Set([...state.selection,...ids]):new Set(ids);}
...
canvas.onpointercancel=()=>endGesture(true);canvas.onlostpointercapture=()=>{if(state.gesture)endGesture(true);};canvas.oncontextmenu=e=>e.preventDefault();
