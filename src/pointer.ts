...
let stopEdgeScroll=()=>{};
export function endGesture(cancel=false){stopEdgeScroll();if(!state.gesture)return;if(cancel&&state.gesture.before&&state.gesture.kind!=='move')state.project=JSON.parse(state.gesture.before);state.gesture=null;canvas.style.cursor='default';if(state.segment)refresh();else{info();layout();}}
...
export function installPointer(){
 ...
 stopEdgeScroll=()=>{if(edgeFrame)cancelAnimationFrame(edgeFrame);edgeFrame=0;edgePoint=null;edgeMotion.reset();};
 const moveSelection=()=>{
 ...
 const resizeSelection=()=>{
  ...
  if(g.cell!==undefined&&tick<g.cell)state.project.notes=stretchBack(g.base,g.nid,cellStart(Math.max(0,tick),state.project.grid),g.cell+128/state.project.grid);
  else state.project.notes=resize(g.base,g.nid,g.length+dx/state.zoom,state.project.grid);
  const note=state.project.notes.find(n=>n.id===g.nid);if(note)extendDragExtent(note.start+note.length);
 };
 const edgeScroll=(time?:number)=>{
  const gesture=state.gesture,p=edgePoint;
  if(!gesture||!['box','move','resize'].includes(gesture.kind)||!p){stopEdgeScroll();return;}
  const {dx,dy}=edgeMotion.step(p,{left:KEY,top:HEAD,right:state.width,bottom:state.height},gesture.kind==='box',time);
  if(dx||dy){view.scrollLeft=Math.max(0,view.scrollLeft+dx);view.scrollTop=Math.max(0,view.scrollTop+dy);
   if(gesture.kind==='move')moveSelection();else{if(gesture.kind==='resize')resizeSelection();info();}draw();
  }
...
canvas.onpointerdown=e=>{
 ...
 if(e.button===0&&e.altKey&&!e.ctrlKey&&!e.metaKey&&!e.shiftKey){
  e.preventDefault();canvas.focus();if(state.gesture)return;
  if(isMuted(state.active)){status('Unmute this instrument to move its notes.');return;}
  const notes=state.project.notes,first=notes.filter(n=>state.selection.has(n.id)).sort((a,b)=>a.start-b.start||a.id-b.id)[0];
  if(!first){status('Select notes, then Alt-click to move them to that time.');return;}
  const target=cellStart(musical(p).tick,state.project.grid);if(target===first.start)return;
  const moved=move(notes,state.selection,first.id,target-first.start,0,state.project.grid);
  if(moved===notes){status('Cannot move the selection there because its instructions would conflict.');return;}
  commitNotes(moved);draw();return;
 }
 const caption=instructionCaptionHit(p);
 let stackNote:Note|undefined;
 if(e.button===0&&!e.ctrlKey&&!e.metaKey&&!e.shiftKey&&!caption&&(state.tool==='select'||state.project.instruments[state.active]?.isInstructions||state.selectedInstruments.size>1)){
  ...
  if(note&&instrumentSelected(note.instrument)&&state.selectedInstruments.size>1){
   stackNote=note;promoteInstrument(note.instrument);state.selection.add(note.id);instruments();
  }else if(note&&note.instrument!==state.active&&(state.tool==='select'||state.project.instruments[state.active]?.isInstructions)){e.preventDefault();canvas.focus();selectInstrument(note.instrument);state.selection=new Set([note.id]);refresh();draw();return;}
 }
 const marker=caption??(!hit(p)?instructionLineHit(p):undefined);if(e.button===0&&marker&&marker.instrument!==state.active){advancedInstructions.enabled=true;state.active=marker.instrument;state.selectedInstruments.clear();state.selection=new Set([marker.id]);refresh();draw();return;}
 if(isMuted(state.active)){status('Unmute this instrument to edit its notes.');return;}e.preventDefault();canvas.focus();
 ...
 if(state.gesture)state.gesture.cycle=cycle;
 if(state.gesture?.kind==='resize')state.gesture.scrollLeft=view.scrollLeft;
 if(state.gesture?.kind==='move'){const g=state.gesture;g.scrollLeft=view.scrollLeft;g.scrollTop=view.scrollTop;g.anchorNote=g.base.find((n:Note)=>n.id===g.anchor);g.maxEnd=g.base.reduce((end:number,n:Note)=>state.selection.has(n.id)?Math.max(end,n.start+n.length):end,0);}
...
canvas.onpointermove=e=>{
 ...
 if(['box','move','resize'].includes(state.gesture.kind)){edgePoint=p;if(!edgeFrame)edgeFrame=requestAnimationFrame(edgeScroll);}
 ...
 if(state.gesture.kind==='resize')resizeSelection();
 if(state.gesture.kind!=='move')info();draw();
};
canvas.onpointerup=e=>{
 ...
 if(state.gesture.kind==='move'&&state.gesture.moved){
  const g=state.gesture;g.current=point(e);moveSelection();
  state.project.notes=move(g.base,state.selection,g.anchor,g.movePreview.dt,g.movePreview.dp,state.project.grid);
 }
 if(!state.gesture.moved&&state.gesture.cycle!==undefined)state.selection=new Set([state.gesture.cycle]);
