...
export type ScaleEstimate={name:string,pitches:number[],coverage:number,alternative:string};
...
export function detectScale(project:Project):ScaleEstimate|null{
 const weights=Array(12).fill(0),volumes=resolveVolumes(project.notes);
 for(const n of project.notes){
  const instrument=project.instruments[n.instrument];
  if(!instrument||instrument.isInstructions||instrument.isDrum||instrument.ms2Drum)continue;
  weights[pitchClass(n.pitch)]+=n.length*(volumes.get(n.id)??8);
 }
 const total=weights.reduce((a,b)=>a+b,0);
 if(!total)return null;
 const candidates=[];
 for(let root=0;root<12;root++)for(const [mode,steps] of [['major',[0,2,4,5,7,9,11]],['minor',[0,2,3,5,7,8,10]]] as const){
