...
import {pitchClass} from './scale.ts';

/** Trim only already-held, sounding notes at an onset that exceeds ten voices. */
export function simplifyHeldNotes(project:Project,index:number,ids?:ReadonlySet<number>){
 const replacements=new Map<number,Note>(),volumes=resolveVolumes(project.notes);
 ...
 let held:Note[]=[],unresolved=0;
 for(let a=0;a<source.length;){
  const tick=source[a].start;let b=a+1;
  while(b<source.length&&source[b].start===tick)b++;
  ...
  if(excess){
   ...
   const cutIds=new Set(cut.map(n=>n.id));held=held.filter(n=>!cutIds.has(n.id));
   for(const n of cut)replacements.set(n.id,{...n,length:tick-n.start});
   if(held.length+fresh.length>10)unresolved++;
  }
  held.push(...fresh);a=b;
 }
...
const instructions=['tempo','timeSignature','section','resetMeasures','loopEntry','loopExit','loopTie','loopCount','speedEntry','speedExit','speedMultiplier'] as const;
/** Exact onset/duration groups retain their outer pitches, regardless of density. */
export function simplifyChords(project:Project,index:number,ids?:ReadonlySet<number>,complexity=2,scale?:readonly number[]){
 if(!Number.isSafeInteger(complexity)||complexity<2)throw Error('Chord Complexity must be a whole number of at least 2.');
 if(project.instruments[index]?.isInstructions)return {notes:project.notes,changed:0,removed:0,protected:0};
 const groups=new Map<string,Note[]>(),removed=new Set<number>(),replacements=new Map<number,Note>();let protectedCount=0;
 for(const n of project.notes)if(n.instrument===index&&(!ids||ids.has(n.id))){const key=`${n.start}:${n.length}`,group=groups.get(key)??[];group.push(n);groups.set(key,group);}
 for(const group of groups.values()){
  group.sort((a,b)=>a.pitch-b.pitch||a.id-b.id);
  if(group.length<=complexity)continue;
  ...
  const width=Math.max(1,high.pitch-low.pitch),parents:number[][]=[];
  let costs=group.map((_,i)=>i===0?0:Infinity);
  for(let slot=1;slot<complexity-1;slot++){
   const next=group.map(()=>Infinity),parent=group.map(()=>-1);let best=Infinity,bestIndex=-1;
   for(let i=1;i<group.length-1;i++){
    if(costs[i-1]<best){best=costs[i-1];bestIndex=i-1;}
  ...
  if(complexity>2){
   let last=1;for(let i=2;i<group.length-1;i++)if(costs[i]<costs[last])last=i;
   for(let slot=parents.length-1;slot>=0;slot--){keep.add(group[last].id);last=parents[slot][last];}
  }
  for(const n of group){
   ...
   if(instructions.some(key=>n[key]!=null&&target[key]!=null&&n[key]!==target[key])){protectedCount++;continue;}
   const payload=Object.fromEntries(instructions.filter(key=>n[key]!=null).map(key=>[key,n[key]]));
   if(Object.keys(payload).length)replacements.set(low.id,{...target,...payload});
   removed.add(n.id);
  }
 }
 if(!removed.size)return {notes:project.notes,changed:0,removed:0,protected:protectedCount};
