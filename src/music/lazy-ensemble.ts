 ...
 */
export function createLazyEnsemble(source:Project,indexes=source.instruments.map((_,i)=>i),minimumEnd=0,extremeCompression=false,limit=10000){
 ...
 const merged:Project={...project,instruments:[{name:'Lazy Ensemble',color:'#abcdef'}],notes:[]};
 const tempos=tempoMap(project.notes,false),speeds=speedMap(project.notes);
 const render=(notes:Note[],warnings=false)=>generateMml(merged,0,notes,tempos,{endTick:expanded.end,speeds,extremeCompression,compactRests:true,skipWarnings:!warnings});
 const fits=(result:MmlResult)=>result.channels.length<=CHANNELS&&result.bytes<=limit;
 ...
 if(!notes.length)return {parts:[],end:expanded.end,warnings:expanded.warnings};
 const voices=partitionChannels(notes);
 if(voices.length>CHANNELS*PLAYERS)throw tooMany();
 const whole=render(notes,true);
 if(fits(whole))return {parts:[whole],end:expanded.end,warnings:[...new Set([...expanded.warnings,...whole.warnings])]};
 type Bin={notes:Note[],result:MmlResult};
 const pack=(ordered:Note[])=>{
  const bins:Bin[]=[];let start=0;
  while(start<ordered.length){
   let count=1,result=render(ordered.slice(start,start+1));
   if(!fits(result))throw Error(`A complete note and its synchronized rests cannot fit within ${limit.toLocaleString()} characters. Lazy Ensemble cannot split held notes without changing their sound.`);
   ...
   let high=2;
   while(start+high<=ordered.length){const next=render(ordered.slice(start,start+high));if(!fits(next))break;count=high;result=next;high*=2;}
   let low=count+1;high=Math.min(high,ordered.length-start);
   while(low<=high){const mid=Math.floor((low+high)/2),next=render(ordered.slice(start,start+mid));if(fits(next)){count=mid;result=next;low=mid+1;}else high=mid-1;}
   bins.push({notes:ordered.slice(start,start+count),result});start+=count;
  }
  // Re-encode unions so disjoint passages reuse channels and controller state.
  for(let i=0;i<bins.length;i++)for(let j=i+1;j<bins.length;){
   const union=[...bins[i].notes,...bins[j].notes],result=render(union);
   if(fits(result)){bins[i]={notes:union,result};bins.splice(j,1);j=i+1;}else j++;
  }
  return bins;
 };
 ...
 if(candidates[0].length>PLAYERS)throw tooMany();
 const parts=candidates[0].map(bin=>render(bin.notes,true));
 if(parts.some(part=>!fits(part)))throw Error('Lazy Ensemble sheet verification failed.');
