...
function lowerBound(notes:Note[],tick:number,edge:(n:Note)=>number){
 ...
 while(low<high){const mid=(low+high)>>>1;if(edge(notes[mid])<tick)low=mid+1;else high=mid;}
 return low;
}

/** Detect ornaments from original timing, before rounding can create a pattern. */
function condense(source:Note[],step:number,end:number){
 const removed=new Set<number>(),replacements=new Map<number,Note>();
 const counts=new Map<number,number>();
 for(const n of source)counts.set(n.start,(counts.get(n.start)??0)+1);
 const joins=(a:Note,b:Note)=>counts.get(a.start)===1&&counts.get(b.start)===1&&b.start>=a.start+a.length&&b.start<=a.start+a.length+1;
 ...
 for(let i=0;i+1<source.length;i++){
  ...
  if(a.length<=4&&b.length>6*a.length&&a.pitch!==b.pitch&&joins(a,b)&&!instructed(a))removed.add(a.id);
 }
 const byEnd=[...source].sort((a,b)=>a.start+a.length-b.start-b.length);
 const short=(n:Note)=>n.length<=4&&n.length<step&&!removed.has(n.id)&&!instructed(n);
 for(let i=0;i<source.length;){
  let j=i+1;
  if(short(source[i]))while(j<source.length&&short(source[j])&&joins(source[j-1],source[j]))j++;
  ...
  if(j-i>=2){
   for(let a=i;a<j;){
    const start=Math.floor(source[a].start/step)*step;let b=a+1;
    while(b<j&&source[b].start<start+step)b++;
    if(b-a>=2&&start+step<=end){
     ...
     if(!collision){
      ...
      for(const n of group.slice(1))removed.add(n.id);
     }
    }
    a=b;
   }
...
export function simplifyTiming(project:Project,index:number,denominator:number,end=Infinity,ids?:ReadonlySet<number>){
 ...
 const edgeStart=(n:Note)=>n.start===down(n.start)||5*Math.min(n.length,down(n.start)+step-n.start)>2*step?down(n.start):down(n.start)+step;
 const edgeEnd=(n:Note)=>{
 ...
 const needsTile=(n:Note)=>n.length<step&&edgeEnd(n)<=edgeStart(n);
 const roundedStart=(n:Note)=>needsTile(n)?down(n.start):edgeStart(n);
 const roundedEnd=(n:Note)=>needsTile(n)?down(n.start)+step:edgeEnd(n);
 const starts=new Map<number,number>(),stops=new Map<number,number>();
 ...
 for(let i=0;i+1<candidates.length;i++){
  ...
  if(aEnd>b.start||down(aEnd-1)!==window||finish>end||instructed(a)||instructed(b))continue;
 ...
 for(const n of candidates)if((stops.get(n.id)??roundedEnd(n))<=(starts.get(n.id)??roundedStart(n))&&(starts.has(n.id)||stops.has(n.id)))condensed.removed.add(n.id);
 ...
 const results=new Map<number,typeof source[number]>();let skipped=0,ended=0,priorEnd=0;
 for(const note of source){
  ...
  while(low<high){const mid=(low+high)>>>1;if(source[mid].start<oldEnd)low=mid+1;else high=mid;}
  let stop=stops.get(note.id)??roundedEnd(note);
  if(!stops.has(note.id)&&low<source.length&&stop>(starts.get(source[low].id)??down(source[low].start)))stop=down(oldEnd);
  if(stop>end)stop=down(end);
  ...
  if(stop<=start||start<priorEnd){results.set(note.id,note);skipped++;continue;}
