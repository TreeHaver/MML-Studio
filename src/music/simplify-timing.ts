...
export function simplifyTiming(project:Project,index:number,denominator:number,end=Infinity){
 ...
 const results=new Map<number,typeof source[number]>();let changed=0,skipped=0,ended=0,priorEnd=0;
 for(const note of source){
  ...
  if(low<source.length&&stop>down(source[low].start))stop=down(oldEnd);
  ...
  results.set(note.id,edited?{...note,start,length:stop-start}:note);if(edited)changed++;
 }
