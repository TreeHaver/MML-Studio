...
export function removeOverlap(project:Project,index:number){
 ...
 for(let a=0;a<source.length;){
  let b=a+1;
  while(b<source.length&&source[b].pitch===source[a].pitch&&source[b].start===source[a].start)b++;
  ...
  let loudest=0;
  for(let i=a;i<b;i++)loudest=Math.max(loudest,volumes.get(source[i].id)!);
  let winner:Note|undefined;
  for(let i=a;i<b;i++){
   const n=source[i],v=volumes.get(n.id)!;
   if(v<loudest-2)continue;
   if(!winner||n.length>winner.length||n.length===winner.length&&v>volumes.get(winner.id)!)winner=n;
  }
  const next=b<source.length&&source[b].pitch===source[a].pitch?source[b].start:Infinity;
  for(let i=a;i<b;i++){
   const note=source[i];
   if(note!==winner){removed.add(note.id);continue;}
   if(note.start+note.length>next)replacements.set(note.id,{...note,length:next-note.start});
  }
  a=b;
 }
