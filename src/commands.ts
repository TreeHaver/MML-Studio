...
import {replaceInheritedTempo} from './model/segment-view.ts';
...
export function commitNotes(notes:Note[]){if(state.segment)notes=replaceInheritedTempo(state.segment.root,state.segment.projection,notes);if(!fitsCurrentView({...state.project,notes})){status('This edit extends beyond the current view. Return to Project to edit across its boundary.');info();return;}if(!valid(notes)){status('That edit would create invalid timing or conflicting tempo, time signature or section instructions.');info();return;}checkpoint();state.project.notes=notes;refresh();}
