...
app.on('browser-window-created',(_,win)=>{if(started)return;started=true;
 win.webContents.once('did-finish-load',async()=>{try{
  ...
  await settled();
  assert.ok(await run(`Array.from(document.getElementById('sound-bank').options).some(o=>o.value==='maplebeats-2.dls')`),'Place the user DLS at assets/maplebeats-2.dls for this native test');
  assert.equal(await run(`import('./dist/playback/engine.js').then(e=>e.activeSoundBank())`),'maplebeats-2.dls');
  assert.equal(await run(`document.getElementById('sound-bank').value`),'maplebeats-2.dls');
  result.checks.push('Fresh profile defaults to Maplebeats with 125 overrides and three GM fallback slots');
  ...
  await run(`const select=document.getElementById('sound-bank');select.value='maplebeats-2.dls';select.dispatchEvent(new Event('change'));`);await settled();
  assert.equal(await run(`e.activeSoundBank()`),'maplebeats-2.dls');
  assert.ok(await run(`e.getEngine().then(engine=>engine.customPrograms.length===125&&engine.customPrograms.includes(127)&&[121,122,123].every(p=>!engine.customPrograms.includes(p)))`),'native bank metadata must distinguish overridden and fallback programs');
  ...
  assert.equal(await run(`localStorage.getItem('mml-studio-sound-bank')`),'maplebeats-2.dls');
  ...
  await run(`document.getElementById('sound-bank').value='maplebeats-2.dls';document.getElementById('sound-bank').dispatchEvent(new Event('change'));`);await settled();
