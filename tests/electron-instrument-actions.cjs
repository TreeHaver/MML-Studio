...
app.on('browser-window-created',(_,win)=>{if(started)return;started=true;win.webContents.once('did-finish-load',async()=>{try{
 ...
 await evaluate(`document.querySelector('.instrument-delete').click()`);assert.equal(await evaluate('JSON.stringify(s.project)'),before);
 ...
 await evaluate(`document.querySelector('.instrument-collapse').click();prefs.muted.add(1);prefs.collapsed.add(2);refresh();
 ...
 await evaluate(`document.getElementById('undo').click()`);assert.equal(await evaluate('JSON.stringify(s.project)'),before);
 await evaluate(`document.getElementById('redo').click();document.querySelector('.instrument-delete').click()`);
 assert.deepEqual(await evaluate('[s.project.instruments.length,s.project.notes.length,s.project.notes[0].instrument]'),[1,1,0]);
 await evaluate(`document.querySelector('.instrument-delete').click()`);
