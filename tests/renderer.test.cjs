...
test('renderer handles click, edge resize, group box/delete, rename, grid and scroll without text nodes on notes',async()=>{
 ...
 const sandbox={document:doc,window:{},devicePixelRatio:1,ResizeObserver:class{observe(){}},structuredClone,confirm:()=>true,console,requestAnimationFrame:fn=>{frame=fn;return 1;},cancelAnimationFrame:()=>{frame=null;}};vm.createContext(sandbox);
 ...
 assert.match(mmlBox().children[0].textContent,/Character count: [1-9][0-9]* bytes/);
 ...
 click(20,240);await new Promise(setImmediate);assert.deepEqual(previewCalls[1],{pitch:expected,program:0,isDrum:true});
 const melodic=doc.getElementById('instruments').children[0].children[2];melodic.value='40';melodic.onchange();
 assert.equal(run('project.instruments[0].isDrum'),false);assert.equal(doc.getElementById('instruments').children[0].children.length,6);
 run('project.instruments.push({name:"Other",color:"#fff"});state.active=1');
 doc.getElementById('view').scrollTop+=20;
 click(20,240);await new Promise(setImmediate);
 assert.deepEqual(previewCalls[2],{pitch:expected-1,program:0});
