...
test('balanced one-unit notes fit half windows without being skipped',()=>{
...
test('two-note roll windows use half windows even at L64; removed competitors preserve V0',()=>{
 for(const denominator of [4,8,16,32,64]){
  const step=128/denominator,r=simplifyTiming(project([note(1,0,step/2),note(2,step/2,step/2)]),0,denominator);
  assert.deepEqual(r.notes.map(n=>[n.start,n.length]),[[0,step/2],[step/2,step/2]]);
 }
 const roll=simplifyTiming(project([0,2,4,6].map((start,i)=>note(i+1,start,2,{pitch:60+i}))),0,32);
 assert.deepEqual(roll.notes.map(n=>[n.start,n.length]),[[0,2],[2,2],[4,2],[6,2]]);
